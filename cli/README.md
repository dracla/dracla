...
| `recipient.slug=SLUG` | defaults to the org with a trailing `-cla` removed, since that organization is conventionally `<project>-cla` and the recipient is `<project>`. Needed only when one org holds pairs for more than one legal recipient (§5.5) |
