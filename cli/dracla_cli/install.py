...
# What DraCLA's own genesis README opens with, per repository. A branch merely
# NAMED `events` is not provenance — an unrelated repository can carry one for
# its own reasons — so reuse requires the branch install creates first to be
# rooted at the file install writes first.
_GENESIS_MARK = {
    EVENTS_REF: "# Canonical CLA records",
    COVERAGE_REF: "# Coverage projection",
}
...
def _provenance(admin, full: str, expected_ref: str) -> str:
    ...
    - The branch must be rooted at the DraCLA genesis README. Install creates
      the branch BY writing that file, so a genuine half-install always has
      it; a branch merely named right does not.
    ...
    try:
        if host.head(expected_ref) is not None:
            readme, _ = host.read(expected_ref, "README.md")
            if readme.startswith(_GENESIS_MARK[expected_ref]):
                return OURS
            return COLLISION
    except NotFound:
...
def run(admin: GitHubAdmin, cfg: InstallConfig, *, version: str,
    ...
    if confirm is not None:
        # A callback that RETURNS a refusal must not read as consent. Signalling
        # only by raising works for the console prompt but silently provisions
        # for any caller that returns a boolean instead.
        if confirm(out.warnings) is False:
            ...
            if verdict == COLLISION:
                raise InstallFailed(
                    ...
                    hint=f"its '{expected_ref.rsplit('/', 1)[-1]}' branch is "
                         f"missing or not rooted at the DraCLA genesis, so "
    ...
    if dry_run:
        # The preview mirrors the real branch below step for step. The fixed
        # three-note list this replaces omitted the coverage section entirely
        # and told a re-run it "would create" a branch that already exists —
        # and a preview that misrepresents state is worse than none, because
        # dry-run's whole purpose is to be trustworthy.
        if cfg.records_repo in out.would_create:
            ...
            try:
                existing, _ = host.read(EVENTS_REF, WORKFLOW_PATH)
                out.note("workflow", Result.UNCHANGED if existing == body
                         else Result.WOULD_UPDATE)
            except NotFound:
                out.note("workflow", Result.WOULD_CREATE)
        if cfg.coverage_repo in out.would_create:
