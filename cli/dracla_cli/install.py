...
def _provenance(admin, full: str) -> str:
    ...
    repository from a name collision with an unrelated one.

    Emptiness is asked of the branch list, never of `size` — GitHub reports
    `size: 0` for plenty of repositories that have content, and believing it
    skipped this check entirely for exactly the repositories most likely to be
    someone else's.
    ...
    for ref in (EVENTS_REF, COVERAGE_REF):
        try:
            ...
            return UNREADABLE            # not `continue`: we cannot tell
...
def run(admin: GitHubAdmin, cfg: InstallConfig, *, version: str,
    ...
    for full, name, desc in (
        ...
        if existing is not None:
            ...
            verdict = _provenance(admin, full)
            ...
            if verdict == COLLISION:
                raise InstallFailed(
                    ...
                    hint=f"it has branches but none that install would have "
                         f"created ('{EVENTS_BRANCH}' or '{COVERAGE_BRANCH}'), "
                         f"so this is a name collision rather than a re-run. "
