...
Keep its membership small. Everyone in that organization can read
contributors' legal names and email addresses, so it should hold the
...
whoever administers agreement versions — rather than the project's
...
def run(admin: GitHubAdmin, cfg: InstallConfig, *, version: str,
    ...
    for full, name, desc in (
        ...
        if admin.get_repo(full) is not None:
    ...
    for path, content in (("source.json", EMPTY_SOURCE),
        ...
        try:
            existing, blob = cov.read(COVERAGE_REF, path)
            if existing != content:
                cov.put(COVERAGE_REF, path, content, base_blob_sha=blob)
        except NotFound:
