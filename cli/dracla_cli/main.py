...
def _confirm(cfg, warnings=()) -> None:
    ...
    # Say what will actually happen. "About to create" is false on a re-run,
    # and consent to create is not consent to modify what is already there.
    verb = "Reusing existing" if warnings else "About to create"
    print(f"{verb} {cfg.records_repo} and {cfg.coverage_repo}, "
          f"both private.", file=sys.stderr)
    try:
