...
class Repo:
    ...
    empty: bool | None      # unknowable from repo metadata; see branch_count()


@dataclass
class CreateRepoResult:
    """Whether POST created the repository or lost a name-creation race."""
...
class GitHubAdmin:
    ...
    def get_repo(self, full_name: str) -> Repo | None:
        ...
        return Repo(full_name=full_name,
                    ...
                    empty=None)          # see branch_count(); size lies

    def create_repo(self, owner: str, name: str,
                    description: str) -> CreateRepoResult:
        """Create a PRIVATE, EMPTY repository.

        Empty on purpose (§6.10.3.1): `auto_init` would create the owner's
        configured default branch, whose name DraCLA does not control. It would
        then have to be demoted and deleted — extra operations, an interval in
        which the default branch is wrong, and a stray branch if the run stops
        in between. The `events` branch becomes the first ref that ever exists
        instead.
        """
        body = {
            "name": name,
        ...
        try:
            ...
            # wrong way.
            if e.status == 422 and "already exists" in e.body.lower():
                ...
                if existing is not None:
                    ...
                    return CreateRepoResult(existing, created=False)
            raise CliError(
        ...
        if not created.private:
            raise CliError(
                ...
                hint="organization policy overrode the private setting. Make it "
                     ...
                     "email addresses.")
        return CreateRepoResult(created, created=True)

    def branch_count(self, full_name: str) -> int | None:
        """How many branches the repository has, or None if that cannot be read.

        NOT `size`. GitHub computes `size` asynchronously in kilobytes and
        reports 0 for plenty of repositories that have content — at the time of
        writing, 179 public repositories with over 200 stars report `size: 0`,
        including one with 11k stars and a README. Treating that as "empty" let
        install seed signer-data scaffolding into a stranger's repository, which
        is exactly what the provenance check exists to prevent.

        A branch listing is the direct question. An empty repository has none.
        """
        try:
            branches = self._get(f"/repos/{full_name}/branches?per_page=2")
            return len(branches) if isinstance(branches, list) else None
        except (NotFound, GitHubError):
            # Cannot tell. The caller must not read that as "empty".
            return None
