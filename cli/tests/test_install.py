...
from dracla.githost import BlobConflict, FakeGitHost                  # noqa: E402

from dracla_cli.admin import CreateRepoResult, Repo                   # noqa: E402
...
from dracla_cli.install import (                                      # noqa: E402
    COVERAGE_README, COVERAGE_REF, EVENTS_BRANCH, EVENTS_REF,
    GENESIS_README, InstallFailed, Result, preflight, run,
)
...
def do_run(admin, host=None, cfg=None, **kw):
    ...
    kw.setdefault("confirm", lambda warnings=(): True)
...
class TestDryRunWritesNothing(unittest.TestCase):
    ...
    def test_preview_covers_every_step_of_a_fresh_install(self):
        """The fixed three-note preview this guards against omitted the whole
        coverage section: an operator reading --dry-run had no idea install
        would seed source/inflight/intents."""
        out, _ = do_run(FakeAdmin(), cfg=dry(["dry_run=true"]))
        whats = {st.what for st in out.steps}
        for expected in ("events branch", "default branch", "workflow",
                         "coverage projection", "coverage source.json",
                         "coverage inflight.json", "coverage intents.json"):
            self.assertIn(expected, whats)
        for st in out.steps:
            self.assertNotIn(st.result, (Result.CREATED, Result.SEEDED,
                                         Result.SET, Result.UPDATED),
                             f"{st.what}: preview may not claim a past-tense write")

    def test_fresh_preview_says_the_first_branch_becomes_default(self):
        out, _ = do_run(FakeAdmin(), cfg=dry(["dry_run=true"]))
        default = next(st for st in out.steps if st.what == "default branch")
        self.assertEqual(default.result, Result.WOULD_BECOME)
        self.assertIn("automatically", default.detail)

        real_admin = FakeAdmin()
        real, _ = do_run(real_admin)
        real_default = next(st for st in real.steps
                            if st.what == "default branch")
        self.assertEqual(real_default.result, Result.UNCHANGED)
        self.assertFalse([w for w in real_admin.writes
                          if w[0] == "set_default_branch"],
                         "preview and real trace must agree there is no PATCH")

    def test_preview_on_reuse_does_not_claim_would_create(self):
        """A re-run preview must report what exists as existing. The old
        preview printed 'events branch: would create' against a repository
        whose events branch it had just verified for provenance."""
        admin = FakeAdmin()
        ...
        admin.repos[CFG.coverage_repo] = Repo(CFG.coverage_repo, True, "coverage", None)
        cov = admin.host_for(CFG.coverage_repo)
        cov.put(COVERAGE_REF, "README.md", COVERAGE_README,
                base_blob_sha=None)
        out, _ = do_run(admin, host=host, cfg=dry(["dry_run=true"]))
        by = {st.what: st.result for st in out.steps}
        self.assertEqual(by["events branch"], Result.EXISTS)
        self.assertEqual(by["default branch"], Result.UNCHANGED)
        self.assertEqual(by["coverage projection"], Result.EXISTS)
        self.assertEqual(by["workflow"], Result.WOULD_CREATE,
                         "absent workflow is the one thing a re-run would add")
        self.assertEqual(by["coverage source.json"], Result.WOULD_CREATE)
        self.assertEqual(admin.writes, [], "the probing preview may not write")
...
class TestBranchLayout(unittest.TestCase):
    ...
    def test_events_is_the_first_ref_and_becomes_default(self):
        ...
        self.assertEqual(admin.get_repo(CFG.records_repo).default_branch,
                         EVENTS_BRANCH)
        self.assertFalse([w for w in admin.writes
                          if w[0] == "set_default_branch"],
                         "GitHub makes the first branch default without PATCH")
    ...
    def test_coverage_projection_is_initialized(self):
        ...
        # The enforcer reads the intent log on every check and an unreadable
        # log fails closed — unseeded, a fresh project could never pass.
        self.assertTrue(cov.exists(COVERAGE_REF, "intents.json"))
        self.assertTrue(cov.exists(COVERAGE_REF, "source.json"))

    def test_repos_are_created_empty(self):
        """auto_init would make an owner-chosen branch we must delete."""
        import inspect
...
class TestWorkflowIsRunnable(unittest.TestCase):
    ...
    def test_flipping_the_flag_requires_settling_provenance_verification(self):
        """Design §9 requires the seeded workflow to verify the release's
        provenance attestation; the template carries that as a recorded M2
        obligation. This tripwire makes the flip fail until a human settles
        it — by implementing verification and removing the obligation comment
        — instead of shipping the template with the TODO still in it."""
        if RECONCILE_IMPLEMENTED:
            self.assertNotIn(
                "M2 must add provenance verification", RECONCILE,
                "the flag is on but the provenance obligation is still an "
                "unimplemented comment; §9's supply-chain rule is not met")
    ...
    def test_the_real_reconciler_pins_actions_by_digest(self):
        """Design §9: the seeded workflow pins actions by digest, never a
        floating tag. It runs in the adopter's PII repository on a runner
        holding the coverage deploy key, so a mutable reference means one push
        to the action's repo executes attacker-chosen code in every adopter's
        records repo at once."""
        body = RECONCILE % {"version": "0.0.1", "events_branch": EVENTS_BRANCH,
                            "coverage_repo": "acme/acme-cla-coverage"}
        uses = [l.strip() for l in body.splitlines() if "uses:" in l]
        self.assertTrue(uses, "template should invoke actions")
        for line in uses:
            with self.subTest(uses=line):
                self.assertRegex(
                    line, r"uses: [^@\s]+@[0-9a-f]{40}\b",
                    "actions must be pinned to a 40-hex commit digest")
...
class TestFailureReporting(unittest.TestCase):
    ...
    def test_every_content_write_preserves_the_outcome(self):
        # records: README, workflow; coverage: README and three state seeds.
        for which, fail_call in (("records", 1), ("records", 2),
                                 ("coverage", 1), ("coverage", 2),
                                 ("coverage", 3), ("coverage", 4)):
            with self.subTest(which=which, fail_call=fail_call):
                admin = FakeAdmin()
                records = admin.host_for(CFG.records_repo)
                coverage = admin.host_for(CFG.coverage_repo)
                target = records if which == "records" else coverage
                original = target.put
                calls = {"n": 0}
                ...
                def fail_at_boundary(*args, **kwargs):
                    ...
                    if calls["n"] == fail_call:
                        raise BlobConflict(f"{which} write {fail_call}")
                    return original(*args, **kwargs)

                target.put = fail_at_boundary
                with self.assertRaises(InstallFailed) as ctx:
                    do_run(admin, host=records)
                outcome = ctx.exception.outcome
                self.assertIsNotNone(outcome)
                self.assertEqual(len(outcome.created), 2)
                self.assertTrue(outcome.steps,
                                "successful steps before the write must survive")
                self.assertIn("re-run", ctx.exception.hint or "")

    def test_default_branch_write_preserves_the_outcome(self):
        admin = FakeAdmin()
        admin.repos[CFG.records_repo] = Repo(
            CFG.records_repo, True, "main", None)
        host = admin.host_for(CFG.records_repo)
        host.put(EVENTS_REF, "README.md", GENESIS_README,
                 base_blob_sha=None)

        def fail_default_branch(full_name, branch):
            raise RuntimeError("default branch update failed")

        admin.set_default_branch = fail_default_branch
        with self.assertRaises(InstallFailed) as ctx:
            do_run(admin, host=host)
        self.assertIsNotNone(ctx.exception.outcome)
        self.assertTrue(ctx.exception.outcome.steps)
        self.assertIn("re-run", ctx.exception.hint or "")
...
class TestGateOrdering(unittest.TestCase):
    ...
    def test_base_permission_is_rechecked_after_confirmation(self):
        admin = FakeAdmin(base_permission="none")

        def change_permission_while_prompt_is_open(warnings=()):
            admin._base = "read"
            return True
        ...
        with self.assertRaises(CliError) as ctx:
            run(admin, CFG, version="0.0.1",
                records_host=admin.host_for(CFG.records_repo),
                coverage_host=admin.host_for(CFG.coverage_repo),
                confirm=change_permission_while_prompt_is_open)
        self.assertIn("every member", str(ctx.exception))
        self.assertEqual(admin.writes, [],
                         "the post-confirmation gate precedes every write")
    ...
    def test_reinstall_does_not_erase_live_intents(self):
        """Same rule as source/inflight: the intent log is live state once an
        activation is in flight; a re-run must seed only when absent."""
        admin = FakeAdmin()
        do_run(admin)
        cov = admin.host_for(CFG.coverage_repo)
        cov.put(COVERAGE_REF, "intents.json",
                '[{"agreement_id": "icla", "version": "v4"}]',
                base_blob_sha=cov.read(COVERAGE_REF, "intents.json")[1])
        do_run(admin)
        self.assertEqual(cov.read(COVERAGE_REF, "intents.json")[0],
                         '[{"agreement_id": "icla", "version": "v4"}]')
    ...
    def test_a_branch_merely_named_events_is_not_provenance(self):
        """An unrelated repository can carry an `events` branch for its own
        reasons. Install creates the branch BY writing the genesis README, so
        a genuine half-install always has it — a name-only match must refuse
        rather than seed a stranger's repository."""
        admin = FakeAdmin()
        admin.repos[CFG.records_repo] = Repo(CFG.records_repo, True, "events", None)
        host = admin.host_for(CFG.records_repo)
        host.put(EVENTS_REF, "README.md", "# Someone else's automation",
                 base_blob_sha=None)
        with self.assertRaises(CliError) as ctx:
            do_run(admin, host=host)
        self.assertIn("not a DraCLA repository", str(ctx.exception))

    def test_a_matching_current_readme_does_not_rewrite_provenance(self):
        """The public marker can be copied after an unrelated root commit."""
        admin = FakeAdmin()
        admin.repos[CFG.records_repo] = Repo(
            CFG.records_repo, True, "events", None)
        host = admin.host_for(CFG.records_repo)
        host.put(EVENTS_REF, "README.md", "# Unrelated repository\n",
                 base_blob_sha=None)
        _, blob = host.read(EVENTS_REF, "README.md")
        host.put(EVENTS_REF, "README.md", GENESIS_README,
                 base_blob_sha=blob)
    ...
    def test_genesis_root_must_contain_only_the_canonical_readme(self):
        admin = FakeAdmin()
        admin.repos[CFG.records_repo] = Repo(
            CFG.records_repo, True, "events", None)
        host = admin.host_for(CFG.records_repo)
        root = host.commit(
            parent=None,
            changes={"README.md": GENESIS_README, "unrelated.txt": "keep me"},
            message="unrelated root")
        host.update_ref(EVENTS_REF, root)

        with self.assertRaises(InstallFailed) as ctx:
            do_run(admin, host=host)
        self.assertIn("not a DraCLA repository", str(ctx.exception))

    def test_422_race_winner_is_checked_for_provenance(self):
        admin = FakeAdmin()
        host = admin.host_for(CFG.records_repo)

        def lose_creation_race(owner, name, description):
            full = f"{owner}/{name}"
            repo = Repo(full, True, "events", None)
            admin.repos[full] = repo
            host.put(EVENTS_REF, "README.md", "# Race winner\n",
                     base_blob_sha=None)
            return CreateRepoResult(repo, created=False)

        admin.create_repo = lose_creation_race
        with self.assertRaises(InstallFailed) as ctx:
            do_run(admin, host=host)
        self.assertIn("not a DraCLA repository", str(ctx.exception))
        self.assertEqual(ctx.exception.outcome.created, [])
        self.assertFalse(host.exists(EVENTS_REF, WORKFLOW_PATH),
                         "the race winner must not receive install writes")

    def test_the_wrong_branch_name_is_not_provenance_either(self):
        """A records candidate whose only branch is `coverage` was once read
        as ours — either expected branch accepted for either repository."""
        admin = FakeAdmin()
        admin.repos[CFG.records_repo] = Repo(CFG.records_repo, True, "coverage", None)
        host = admin.host_for(CFG.records_repo)
        host.put(COVERAGE_REF, "README.md", "# Coverage projection\n",
                 base_blob_sha=None)
        admin.branch_counts[CFG.records_repo] = 1
        with self.assertRaises(CliError) as ctx:
            do_run(admin, host=host)
    ...
    def test_a_genuine_half_install_with_genesis_is_reused(self):
        """The recovery path the provenance rules must not break: events branch
        rooted at the DraCLA genesis README."""
        admin = FakeAdmin()
        admin.repos[CFG.records_repo] = Repo(CFG.records_repo, True, "events", None)
        host = admin.host_for(CFG.records_repo)
        host.put(EVENTS_REF, "README.md", GENESIS_README, base_blob_sha=None)
        out, _ = do_run(admin, host=host)
        self.assertIn(("workflow", ), [(st.what,) for st in out.steps
                                       if st.what == "workflow"])
    ...
    def test_confirmation_requires_literal_true(self):
        for value in (None, 0, "", [], 1, object()):
            with self.subTest(value=value):
                ...
                with self.assertRaises(Aborted):
                    run(admin, CFG, version="0.0.1",
                        confirm=lambda _warnings, value=value: value,
                        records_host=admin.host_for(CFG.records_repo),
                        coverage_host=admin.host_for(CFG.coverage_repo))
                self.assertEqual(admin.writes, [])
