...
class TestGateOrdering(unittest.TestCase):
    ...
    def test_an_unrelated_existing_repository_is_not_reused(self):
        ...
        admin = FakeAdmin()
        # private, but NOT empty and with no branch install would have made
        admin.repos[CFG.records_repo] = Repo(CFG.records_repo, True, "main", False)
        with self.assertRaises(CliError) as ctx:
            do_run(admin)
    ...
    def test_our_own_half_created_repository_is_still_reused(self):
        ...
        admin = FakeAdmin()
        admin.repos[CFG.records_repo] = Repo(CFG.records_repo, True, None, True)
        out, _ = do_run(admin)
...
class TestOrgGuidance(unittest.TestCase):
    ...
    def test_the_gate_says_who_should_be_a_member(self):
        with self.assertRaises(CliError) as ctx:
            preflight(FakeAdmin(base_permission="read"), CFG)
        hint = ctx.exception.hint or ""
        self.assertIn("Keep its membership small", hint)
