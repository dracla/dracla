...
class TestEventPreconditions(unittest.TestCase):
    ...
    def _activation_event(self, version, supersedes_coverage, accepted_versions):
    ...
    def _preconditions(self, event, head=HEAD, overrides=None):
        ...
        for requirement in required_preconditions(event, expected_head=head):
            ...
            if override_name in overrides:
            ...
            elif requirement.name in {"published-agreement", "target-activation", "scope-request", "source-event", "rule-event"}:
                ...
                linked_type = {"published-agreement": "agreement_published", "target-activation": "agreement_activated", "scope-request": "enforcement_scope_requested", "source-event": "exemption", "rule-event": "exemption_rule_configured"}[requirement.name]
                ...
                elif requirement.name == "target-activation":
                ...
                if event.type == "enforcement_scope_requested":
                    ...
                    request_payload = self.rows["enforcement_scope_requested"]["payload"]
                    entry["enforcement_scope"] = request_payload["prior_scope"]
                    entry["registry_generation"] = request_payload["prior_registry_generation"]
                    staged = copy.deepcopy(entry)
                    staged["enforcement_scope"] = event.payload["desired_scope"]
                    staged["registry_generation"] = event.payload["registry_generation"]
                    staged["request_event_links"][event.target["change_id"]] = event.payload["request_event_id"]
                    value = {"prior_entry": entry, "staged_entry": staged}
                else:
                ...
                if event.type == "records_reader_materialized":
                    value.update(
                        ...
                        subject=dict(event.target["subject"]),
                ...
                if event.type == "records_reader_withdrawn":
                    value.update(source_event_id=source_id, subject=dict(event.target["subject"]))
                elif event.type == "records_reader_rule_withdrawn":
                    value["rule_event_id"] = source_id
    ...
    def test_activation_prior_projection_vectors(self):
        ...
        malformed = {
            ...
            "duplicate_retired": {**projections["v2_after_supersession"], "retired_versions": ["1", "1"]},
            "overlapping_versions": {**projections["v1"], "retired_versions": ["1"]},
            "unsorted_accepted": {**projections["v1"], "accepted_versions": ["2", "1"], "active_version": "1"},
            "unsorted_retired": {**projections["v2_after_supersession"], "retired_versions": ["3", "1"]},
            "bad_retired_versions": {**projections["v1"], "retired_versions": "bad"},
            "non_string_retired_version": {**projections["v1"], "retired_versions": [1]},
            "empty_with_retired": {**projections["empty"], "retired_versions": ["1"]},
        }
        ...
        for case in self.corpus["activation_cases"]:
            with self.subTest(case=case["id"]):
                ...
                if "accepted_versions" in case:
                    accepted_versions = case["accepted_versions"]
                elif case["expect"] == "accept":
                    accepted_versions = (
                        [case["event_version"]]
                        if case["supersedes_coverage"]
                        else sorted(set(projection["accepted_versions"]) | {case["event_version"]})
                    )
                else:
                    accepted_versions = [case["event_version"]]
                event = self._activation_event(case["event_version"], case["supersedes_coverage"], accepted_versions)
    ...
    def test_restore_validates_exact_target_and_derives_currency_transition(self):
        restore = validate_event(self.rows["agreement_activation_restored"])
        values = self._preconditions(restore)
        evidence = event_module._validate_preconditions(restore, values, HEAD)
        self.assertEqual(
            event_module._expected_active_agreement(restore, evidence),
            {
                "agreement_id": "agreement-1",
                "active_version": "1",
                "accepted_versions": ["1"],
                "retired_versions": ["2"],
                "activation_event_id": restore.event_id,
                "projection_format": 1,
                "shard_count": 32,
            },
        )
        ...
        active = next(item["value"] for item in values if item["name"] == "active-agreement")
        mutations = {
            "target_is_restore": {**target, "type": "agreement_activation_restored"},
            "wrong_target_event": {**target, "event_id": self.rows["agreement_published"]["event_id"]},
            "wrong_target_agreement": {**target, "target": {**target["target"], "agreement_id": "other"}},
            "malformed_target": {**target, "target": {"agreement_id": "agreement-1"}},
            "malformed_payload": {**target, "payload": {}},
            "empty_target_set": {**target, "payload": {**target["payload"], "accepted_versions": []}},
            "malformed_supersedes_flag": {**target, "payload": {**target["payload"], "supersedes_coverage": 1}},
            "target_version_not_accepted": {**target, "payload": {**target["payload"], "accepted_versions": ["2"]}},
        }
        for name, replacement in mutations.items():
            with self.subTest(name=name), self.assertRaises(PreconditionValidationError):
                event_module._validate_preconditions(
                    restore,
                    self._preconditions(restore, overrides={"target-activation": replacement}),
                    HEAD,
                )

        mismatched = replace(
            restore,
            payload=event_module._freeze({"accepted_versions": ["2"], "reason": restore.payload["reason"]}),
        )
        with self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(mismatched, self._preconditions(mismatched), HEAD)

        multi = replace(
            restore,
            payload=event_module._freeze({"accepted_versions": ["1", "2"], "reason": restore.payload["reason"]}),
        )
        multi_target = {
            **target,
            "payload": {**target["payload"], "accepted_versions": ["1", "2"]},
        }
        multi_active = {
            **active,
            "active_version": "3",
        ...
        multi_values = self._preconditions(
            multi,
            overrides={"target-activation": multi_target, "active-agreement": multi_active},
        )
        multi_evidence = event_module._validate_preconditions(multi, multi_values, HEAD)
        self.assertEqual(
            event_module._expected_active_agreement(multi, multi_evidence)["retired_versions"],
            ["3"],
        )
        with self.assertRaises(PreconditionValidationError):
            event_module._expected_active_agreement(validate_event(self.rows["config_updated"]), {})
    ...
    def test_lifecycle_and_confirmed_head_vectors(self):
        ...
        for case in self.corpus["lifecycle_cases"]:
            ...
            if "authorization_operation" in case:
                operation = case["authorization_operation"]
                ...
                authorization = AuthorizationEvidence(
                    operation, kind, resource_id, authority, "admin", True,
                    "2026-08-29T00:00:00Z", None,
                )
                event = replace(event, authorizations=(authorization,))
    ...
    def test_registry_owner_and_scope_relations(self):
        ...
        mutations = ("project", "prior_owner", "staged_owner", "routing", "repository_drift", "scope", "links", "generation", "repositories", "slug", "entry_generation")
        for mutation in mutations:
            ...
            elif mutation == "repository_drift":
                value["staged_entry"]["repository_ids"] = {"records": 21, "coverage": 22, "control": 23}
            elif mutation == "scope":
                value["staged_entry"]["enforcement_scope"] = self.rows["enforcement_scope_requested"]["payload"]["desired_scope"]
            elif mutation == "links":
                value["staged_entry"]["request_event_links"]["other-change"] = owner.event_id
        ...
        for mutation in ("scope", "link", "owner", "slug", "repositories", "unrelated_link", "prior_link", "prior_scope", "prior_generation", "binding"):
            ...
            if mutation == "scope":
                ...
                item["value"]["staged_entry"]["request_event_links"][activated.target["change_id"]] = owner.event_id
            elif mutation == "owner":
                item["value"]["staged_entry"]["repository_owner"] = owner.payload["new_repository_owner"]
            elif mutation == "slug":
                item["value"]["staged_entry"]["project_slug"] = "other-project"
            elif mutation == "repositories":
                item["value"]["staged_entry"]["repository_ids"] = {"records": 21, "coverage": 22, "control": 23}
            elif mutation == "unrelated_link":
                item["value"]["staged_entry"]["request_event_links"]["other-change"] = owner.event_id
            elif mutation == "prior_link":
                item["value"]["prior_entry"]["request_event_links"][activated.target["change_id"]] = activated.payload["request_event_id"]
            elif mutation == "prior_scope":
                item["value"]["prior_entry"]["enforcement_scope"] = activated.payload["desired_scope"]
            elif mutation == "prior_generation":
                item["value"]["prior_entry"]["registry_generation"] += 1
            else:
        ...
        nonadvancing = replace(
            activated,
            payload=event_module._freeze({
                **event_module._thaw(activated.payload),
                "registry_generation": request_payload["prior_registry_generation"],
            }),
        )
        with self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(nonadvancing, self._preconditions(nonadvancing), HEAD)
    ...
    def test_reader_and_materialization_facts_are_authenticated(self):
        ...
        for replacement in (
            {"project_id": reader.project_id, "class_state": "current", "source_event_ids": [], "cursor_event_id": reader.event_id, "source_event_id": reader.target["source_event_id"], "subject": reader.target["subject"]},
            {"project_id": reader.project_id, "class_state": "current", "source_event_ids": ["bad"], "cursor_event_id": reader.event_id, "source_event_id": reader.target["source_event_id"], "subject": reader.target["subject"]},
            {"project_id": reader.project_id, "class_state": "current", "source_event_ids": [reader.target["source_event_id"], reader.target["source_event_id"]], "cursor_event_id": reader.event_id, "source_event_id": reader.target["source_event_id"], "subject": reader.target["subject"]},
        ):
            values = self._preconditions(reader)
    ...
    def test_reader_and_exemption_facts_reject_wrong_shapes(self):
        ...
        values = self._preconditions(reader)
        item = next(item for item in values if item["name"] == "reader-authority-state")
        item["value"] = {"project_id": "other", "class_state": "current", "source_event_ids": [], "cursor_event_id": reader.event_id, "source_event_id": reader.target["source_event_id"], "subject": reader.target["subject"]}
        with self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(reader, values, HEAD)
        authorized = validate_event(self.rows["records_reader_authorized"])
        values = self._preconditions(authorized)
    ...
    def test_exemption_source_withdrawal_accepts_known_absent_source(self):
        source = validate_event(self.rows["exemption_source_withdrawn"])
        event_module._validate_preconditions(source, self._preconditions(source), HEAD)

        absent = self._preconditions(
            source,
            overrides={
                "current-exemption-union-07": {
        ...
        event_module._validate_preconditions(source, absent, HEAD)

        other_source = self._preconditions(
            source,
            overrides={
                "current-exemption-union-07": {
                    "project_id": source.project_id,
                    "subjects": [dict(source.target["subject"])],
                    "provenance": {"7": [self.rows["exemption_snapshot"]["event_id"]]},
                }
            },
        )
        event_module._validate_preconditions(source, other_source, HEAD)
    ...
    def test_reader_rule_profile_joins_prior_reader_authority_generation(self):
        event = validate_event(self.rows["records_reader_rule_configured"])
        values = self._preconditions(event)
        prior = next(item for item in values if item["name"] == "prior-generations")
        prior["value"]["status_detail"] = self.rows["revocation"]["event_id"]
        event_module._validate_preconditions(event, values, HEAD)

        values = self._preconditions(event)
        prior = next(item for item in values if item["name"] == "prior-generations")
        prior["value"]["reader_authority"] = self.rows["revocation"]["event_id"]
        with self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(event, values, HEAD)

    def test_reader_materialization_and_affected_classes(self):
        for event_type, target_key in (("records_reader_withdrawn", "source_event_id"), ("records_reader_rule_withdrawn", "rule_event_id")):
            withdrawal = validate_event(self.rows[event_type])
            target_id = withdrawal.target[target_key]
            for source_ids, state, expected in (([target_id], "current", ("reader_authority",)), ([], "current", ()), ([], "stale", ("reader_authority",))):
                observed_generation = withdrawal.event_id if state == "current" else self.rows["revocation"]["event_id"]
                fact = {
                    "project_id": withdrawal.project_id,
                    "class_state": state,
                    "source_event_ids": source_ids,
                    "cursor_event_id": observed_generation,
                    target_key: target_id,
                }
                if event_type == "records_reader_withdrawn":
                    fact["subject"] = dict(withdrawal.target["subject"])
                values = self._preconditions(withdrawal, overrides={"reader-authority-state": fact})
                observation = next(item for item in values if item["name"] == "reader-authority-state")
                observation["binding"]["derived_generation"] = observed_generation
                with self.subTest(event_type=event_type, state=state, source_ids=source_ids):
    ...
    def test_reader_withdrawal_state_binds_exact_target_and_observed_generation(self):
        alternate = self.rows["exemption"]["event_id"]
        self.assertEqual(
            event_module._reader_shard_path(alternate),
            event_module._reader_shard_path(self.rows["records_reader_withdrawn"]["target"]["source_event_id"]),
        )
        for event_type, target_key in (("records_reader_withdrawn", "source_event_id"), ("records_reader_rule_withdrawn", "rule_event_id")):
            withdrawal = validate_event(self.rows[event_type])
            valid = self._preconditions(withdrawal)
            event_module._validate_preconditions(withdrawal, valid, HEAD)

            wrong_target = self._preconditions(withdrawal)
            wrong_target_fact = next(item for item in wrong_target if item["name"] == "reader-authority-state")
            wrong_target_fact["value"][target_key] = alternate
            with self.subTest(event_type=event_type, mutation="same-shard-target"), self.assertRaises(PreconditionValidationError):
                event_module._validate_preconditions(withdrawal, wrong_target, HEAD)

            if event_type == "records_reader_withdrawn":
                wrong_subject = self._preconditions(withdrawal)
                wrong_subject_fact = next(item for item in wrong_subject if item["name"] == "reader-authority-state")
                wrong_subject_fact["value"]["subject"] = {"github_user_id": 8, "login_snapshot": "other"}
                with self.subTest(event_type=event_type, mutation="wrong-subject"), self.assertRaises(PreconditionValidationError):
                    event_module._validate_preconditions(withdrawal, wrong_subject, HEAD)

            wrong_cursor = self._preconditions(withdrawal)
            wrong_cursor_fact = next(item for item in wrong_cursor if item["name"] == "reader-authority-state")
            wrong_cursor_fact["value"]["cursor_event_id"] = self.rows["revocation"]["event_id"]
            with self.subTest(event_type=event_type, mutation="wrong-observed-generation"), self.assertRaises(PreconditionValidationError):
                event_module._validate_preconditions(withdrawal, wrong_cursor, HEAD)

    def test_reader_materialization_authority_binds_rule_subject_and_presence(self):
        from dracla.conformance import derive_automation_nonce, derive_event_identity

        def materialization(result):
            row = copy.deepcopy(self.rows["records_reader_materialized"])
            row["payload"]["result"] = result
            row["payload"]["membership_evidence"]["state"] = "member" if result == "add" else "not_member"
            row["operation_nonce"] = derive_automation_nonce(
                row["target"]["rule_event_id"],
                row["payload"]["membership_evidence"]["github_user_id"],
                result,
                row["payload"]["prior_materialization_event_id"],
            )
            identity = derive_event_identity(
                row["project_id"],
                row["operation_nonce"],
        ...
        add = materialization("add")
        alternate = self.rows["exemption"]["event_id"]
        self.assertEqual(
            event_module._reader_shard_path(alternate),
            event_module._reader_shard_path(add.target["rule_event_id"]),
        )
        event_module._validate_preconditions(add, self._preconditions(add), HEAD)

        wrong_rule = self._preconditions(add)
        authority = next(item for item in wrong_rule if item["name"] == "current-reader-authority")
        authority["value"]["rule_event_id"] = alternate
        with self.subTest(mutation="same-shard-rule"), self.assertRaisesRegex(PreconditionValidationError, "reader authority rule event does not match materialization"):
            event_module._validate_preconditions(add, wrong_rule, HEAD)

        wrong_subject = self._preconditions(add)
        authority = next(item for item in wrong_subject if item["name"] == "current-reader-authority")
        authority["value"]["subject"] = {"github_user_id": 8, "login_snapshot": "other"}
        with self.subTest(mutation="different-subject"), self.assertRaisesRegex(PreconditionValidationError, "reader authority subject does not match materialization"):
            event_module._validate_preconditions(add, wrong_subject, HEAD)

        repeated_add = self._preconditions(add)
        authority = next(item for item in repeated_add if item["name"] == "current-reader-authority")
        authority["value"]["sources"] = [add.target["rule_event_id"]]
        with self.subTest(mutation="repeated-add"), self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(add, repeated_add, HEAD)

        substituted_repeated_add = self._preconditions(add)
        authority = next(item for item in substituted_repeated_add if item["name"] == "current-reader-authority")
        authority["value"]["subject"] = {"github_user_id": 8, "login_snapshot": "other"}
        with self.subTest(mutation="repeated-add-substituted-subject"), self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(add, substituted_repeated_add, HEAD)

        inactive_add = self._preconditions(add)
        authority = next(item for item in inactive_add if item["name"] == "current-reader-authority")
        authority["value"]["source_record_present"] = False
        with self.subTest(mutation="inactive-rule-add"), self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(add, inactive_add, HEAD)

        malformed_presence = self._preconditions(add)
        authority = next(item for item in malformed_presence if item["name"] == "current-reader-authority")
        authority["value"]["source_record_present"] = 1
        with self.subTest(mutation="malformed-source-record-presence"), self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(add, malformed_presence, HEAD)

        inconsistent_presence = self._preconditions(add)
        authority = next(item for item in inconsistent_presence if item["name"] == "current-reader-authority")
        authority["value"]["sources"] = [add.target["rule_event_id"]]
        authority["value"]["source_record_present"] = False
        with self.subTest(mutation="inconsistent-source-record-presence"), self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(add, inconsistent_presence, HEAD)

        withdraw = materialization("withdraw")
        event_module._validate_preconditions(withdraw, self._preconditions(withdraw), HEAD)

        missing_withdraw = self._preconditions(withdraw)
        authority = next(item for item in missing_withdraw if item["name"] == "current-reader-authority")
        authority["value"]["sources"] = []
        with self.subTest(mutation="missing-withdraw"), self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(withdraw, missing_withdraw, HEAD)

        substituted_withdraw = self._preconditions(withdraw)
        authority = next(item for item in substituted_withdraw if item["name"] == "current-reader-authority")
        authority["value"]["subject"] = {"github_user_id": 8, "login_snapshot": "other"}
        with self.subTest(mutation="missing-withdraw-substituted-subject"), self.assertRaises(PreconditionValidationError):
            event_module._validate_preconditions(withdraw, substituted_withdraw, HEAD)
    ...
    def test_event_link_relations_are_closed(self):
        ...
        for event_type, snapshot_type in (
            ("exemption_source_withdrawn", "exemption_snapshot"),
            ("records_reader_withdrawn", "records_reader_snapshot_authorized"),
        ):
            withdrawal = validate_event(self.rows[event_type])
            linked = next(item["value"] for item in self._preconditions(withdrawal) if item["name"] == "source-event")
            renamed = {**withdrawal.target["subject"], "login_snapshot": "renamed-contributor"}
            direct = {**linked, "target": {"subject": renamed}}
            snapshot = {**linked, "type": snapshot_type, "target": {"subjects": [renamed]}}
            with self.subTest(event_type=event_type, source_kind="direct"):
                event_module._validate_link_fact(withdrawal, "source-event", direct)
            with self.subTest(event_type=event_type, source_kind="snapshot"):
                event_module._validate_link_fact(withdrawal, "source-event", snapshot)
