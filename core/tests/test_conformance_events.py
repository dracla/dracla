...
class TestEventModel(unittest.TestCase):
    ...
    def test_precondition_descriptors_resolve_to_deterministic_artifact_paths(self):
        ...
        acceptance_active_agreement = next(
            item for item in required_preconditions(acceptance, expected_head=acceptance.confirmed_canonical_oid)
            if item.name == "active-agreement"
        )
        self.assertEqual(acceptance_active_agreement.relation, "active-agreement-matches-target")
        activation = validate_event(self.rows["agreement_activated"])
        activation_active_agreement = next(
            item for item in required_preconditions(activation, expected_head=HEAD)
            if item.name == "active-agreement"
        )
        self.assertEqual(activation_active_agreement.relation, "prior-active-agreement-and-prospective-activation-match")
        restore = validate_event(self.rows["agreement_activation_restored"])
        restore_descriptors = {
            item.name: item for item in required_preconditions(restore, expected_head=HEAD)
        }
        self.assertEqual(
            restore_descriptors["target-activation"].path,
            event_module.event_path(restore.target["activation_event_id"]),
        )
        self.assertEqual(
            restore_descriptors["target-activation"].relation,
            "target-activation-exists-and-matches-restore",
        )
        self.assertEqual(
            restore_descriptors["active-agreement"].relation,
            "prior-active-agreement-and-prospective-restore-match",
        )
