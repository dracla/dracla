...
from dracla.conformance.artifacts import resolve_artifact_identity  # noqa: E402
...
class TestEventModel(unittest.TestCase):
    ...
    def test_precondition_declaration_matrix_is_exact_for_every_event(self):
        ...
        for value in self.rows.values():
            ...
            descriptors = required_preconditions(event, expected_head=HEAD)
            self.assertEqual(
            ...
            self.assertEqual(
                descriptors,
                required_preconditions(event, expected_head=HEAD),
            )
            for descriptor in descriptors:
                with self.subTest(event_type=event.type, name=descriptor.name):
                    self.assertEqual(descriptor.expected_head, HEAD)
                    self.assertIn(
                    ...
                    self.assertTrue(descriptor.path)
    ...
    def test_precondition_descriptors_resolve_to_deterministic_artifact_paths(self):
        for value in self.rows.values():
            event = validate_event(value)
            for descriptor in required_preconditions(event, expected_head=HEAD):
                with self.subTest(event_type=event.type, name=descriptor.name):
        ...
        self.assertEqual(
            next(item for item in required_preconditions(revocation, expected_head=HEAD) if item.name == "coverage-state-07").path,
            "users/07.enc.json",
        )
