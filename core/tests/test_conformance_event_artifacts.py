...
class TestSideArtifactPackages(unittest.TestCase):
    ...
    def test_currency_events_have_no_events_branch_side_artifact(self):
        for event_type, expected in (
            ...
            self.assertEqual(required_side_artifacts(event, preconditions=preconditions, expected_head=HEAD), ())
            validate_side_artifact_package(event, [], preconditions=preconditions, expected_head=HEAD)
            artifact = SideArtifact("active_agreement", "agreements/active.enc.json", canonical_json(projection))
            with self.subTest(event_type=event_type), self.assertRaises(SideArtifactValidationError):
        ...
        self.assertEqual(required_side_artifacts(superseding, preconditions=preconditions, expected_head=HEAD), ())
        validate_side_artifact_package(superseding, [], preconditions=preconditions, expected_head=HEAD)
        artifact = SideArtifact("active_agreement", "agreements/active.enc.json", canonical_json(projection))
        with self.assertRaises(SideArtifactValidationError):
