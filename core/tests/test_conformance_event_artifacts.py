...
class TestSideArtifactPackages(unittest.TestCase):
    ...
    def _preconditions(self, event, head=HEAD, overrides=None):
        ...
        for requirement in required_preconditions(event, expected_head=head):
            ...
            if override_name in overrides:
                ...
                linked_type = {"published-agreement": "agreement_published", "scope-request": "enforcement_scope_requested", "source-event": "exemption", "rule-event": "exemption_rule_configured"}[name]
    ...
    def test_artifact_model_and_ordered_package_membership(self):
        ...
        self.assertEqual(artifact.sha256, "sha256:" + hashlib.sha256(b"{}").hexdigest())
        self.assertIs(event_module._coerce_side_artifact(artifact), artifact)
        decoded = event_module._coerce_side_artifact({"kind": "project_config", "path": "config/project.enc.json", "bytes_b64": "e30"})
        ...
        for value in (
            None,
            {"kind": "project_config", "path": "config/project.enc.json", "bytes": b"{}", "extra": True},
            {"kind": "project_config", "path": "config/project.enc.json"},
            {"kind": "project_config", "path": "config/project.enc.json", "bytes": "{}"},
            {"kind": "project_config", "path": "config/project.enc.json", "bytes_b64": "!"},
            {"kind": [], "path": "config/project.enc.json", "bytes": b"{}"},
        ):
