...
class TestAuthorizationVocabulary(unittest.TestCase):
    ...
    def test_scope_activation_and_abandonment_keep_one_literal_relation(self):
        repository = lambda resource_id: {
            "kind": "repository", "repository_id": resource_id,
            "owner_snapshot": "owner", "name_snapshot": f"repo-{resource_id}",
        }
        organization = lambda resource_id: {
            "kind": "organization", "organization_id": resource_id,
            "login_snapshot": f"org-{resource_id}",
        }
        cases = (
            ("enforcement_scope_repository_bind", "repository", 31, [repository(31)], []),
            ("enforcement_scope_repository_widen", "repository", 32, [repository(31), repository(32)], [repository(31)]),
            ("enforcement_scope_repository_narrow", "repository", 32, [repository(31)], [repository(31), repository(32)]),
            ("enforcement_scope_repository_remove", "repository", 31, [], [repository(31)]),
            ("enforcement_scope_organization_bind", "organization", 41, [organization(41)], []),
            ("enforcement_scope_organization_widen", "organization", 42, [organization(41), organization(42)], [organization(41)]),
            ("enforcement_scope_organization_narrow", "organization", 42, [organization(41)], [organization(41), organization(42)]),
            ("enforcement_scope_organization_remove", "organization", 41, [], [organization(41)]),
        )
        for operation, kind, resource_id, desired_scope, inverse_scope in cases:
            authority = "organization_owner" if kind == "organization" else "contributing_repository_admin"
            item = _evidence(operation, kind, resource_id, authority)
            with self.subTest(operation=operation, polarity="valid"):
                self.assertEqual(
                    validate_authorizations(
                        "enforcement_scope_activated", {"change_id": "c"},
                        {"desired_scope": desired_scope}, self.actor, [item],
                    ),
                    (item,),
                )
            with self.subTest(operation=operation, polarity="inverse"):
                with self.assertRaises(AuthorizationValidationError):
                    validate_authorizations(
                        "enforcement_scope_activated", {"change_id": "c"},
                        {"desired_scope": inverse_scope}, self.actor, [item],
                    )

        selector = repository(55)
