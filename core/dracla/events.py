...
def validate(event: Event, *, config: dict[str, Any]) -> None:
    ...
    if event.type == ACCEPTANCE:
        # `config` is the resolved project configuration (config/project.json).
        # Hydra composes it on the CLI client; by the time it reaches here it is
        # a plain dict, and nothing in core depends on Hydra.
