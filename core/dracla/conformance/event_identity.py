...
def _scope_expected(
    ...
    if event_type == "enforcement_scope_activated" and desired is not None:
        ...
        # and resource identity are carried by the authorization row; bind and
        # widen retain the resource, while narrow and remove omit it.  Do not
        # dereference the prior request here.
        present = (kind, item.resource_id) in set(desired)
        must_be_present = action in {"bind", "widen"}
        if present != must_be_present:
            _fail("scope activation resource membership does not match action", authorization=True)
