...
| **M1-18** | TypeScript incremental records-derived updates and portal reads | M1-14, M1-17 | Edge incremental index/status/reader-source updates with bounded bulk fan-out that fails closed until the final generation, plus §6.10.4 read validation of state, all 32 reader shards, generations, digests, and profile limits, and read-time resolution of live coverage marker/fence state so a stale `current`, `revoked`, or `indeterminate` row is never served | Proof issuance, live membership checks, export rendering, hosted jobs |
