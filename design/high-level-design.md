...
- `--dry-run` issues no write of any kind
...
- the organization gate blocks, and the override is the only way past it
...
reason (§8.4.1). The coverage projection carries no personal data at all, which
is what allows the enforcement path to be blind to it (§5.3).
