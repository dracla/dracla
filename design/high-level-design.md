...
and the workflow and predates any event (§6.10.3.1). That reading of
`REQ-REC-3` — it governs event commits, and the pre-event bootstrap carries
none — is declared in §10.3 rather than assumed. It is also invisible to
...
| `REQ-REC-3` | **Interpretation declared** — the one-logical-event-per-commit rule governs event commits; the two pre-event bootstrap commits (README, workflow — §6.10.3.1) carry none, and replay identifies events by `events/**` path, never by commit position (§6.10.1) |
| `REQ-CONFIG-3` | **Constraint acknowledged** — cross-organization scope is definable, but in v1 each entry must be bound by an administrator of its owner (§7); the federated case with no such administrator is deferred, not unconsidered |
