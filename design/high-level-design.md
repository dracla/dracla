...
agreement, exemptions, and subject rows therefore describe one internally
consistent observation; a check never combines an old marker with newer shards
or the reverse. The published check records `C` for audit and retry.
...
- **Subrequests — bounded for an ordinary pull-request check.** It costs route
  ...
  write (1), or 10–43 against a cap of 50. The 32-shard Free layout (D9)
...
- **A2 — Subrequests bounded; maximum-shard, merge-group, and Durable Objects measurement required before release.**
  An ordinary pull-request check uses at most 43 subrequests, including route
  and gate lookup, the immutable coverage-ref snapshot, and all 32 Free-profile
