...
§6.10 and removed after review rather than patched — see
`design/cli-review-findings.md` — and its reimplementation follows this
design.
...
not control. Install instead creates the repository empty and makes `events`
the first ref, so it becomes default without guessing or later promoting a
branch. An earlier implementation assumed the default was `main` and wrote
everything to a branch the reconciler never checked out.
...
**Why empty rather than `auto_init`.** `auto_init` creates whichever default
branch the owner configured, whose name DraCLA does not control. That branch
would then have to be demoted and deleted — two extra operations, an interval
during which the default branch is the wrong one, and a stray branch left
behind if the run stops in between. Creating the repository empty and making
`events` the first ref removes the problem instead of handling it: GitHub sets
the default branch to the only branch that exists.
...
record no consent, because narrowing needs none. An `owner/*` entry is
...
6. invoke the enforcer sweep for open pull requests in scope
```
