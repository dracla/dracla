...
close a marker. Activations carry their own: the intent record (§6.5) is
appended to `intents.json` *before* the canonical commit and settled by the
reconciler, exactly the open-before-commit, orphans-fail-closed lifecycle of
this section — an orphaned intent makes subjects read uncovered, never covered,
until replay confirms or clears it. A guard that only one code path maintains is a liveness
...
pull_request opened / synchronize
  ...
  -> read intents.json: an effective supersedes intent that
     postdates a subject's accepted version, with no operative
     pending early signature                                 -> that subject uncovered  (§6.5)
...
The enforcer overlays unfolded intents at read time: if an effective
`supersedes_coverage: true` intent postdates a subject's accepted version, that
subject is uncovered — instantly, with no rewrite in the path. An early signer
...
canonical triggers one, so folding is prompt — and marks the intent folded.
