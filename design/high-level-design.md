...
Status: Draft
Date: 22 August 2026
Requirements baseline: `design/requirements.md` (Locked, revision 11, 22 August 2026)
...
- `contents: write` on **coverage** (materialization, the in-flight marker,
  active-version updates, and records-ACL status, §5.4)
- Repository `administration: read` on canonical and coverage, plus organization
  `members: read` for organization custody, so the daily `worker-portal` scan
  can enumerate owners, teams, collaborators, and service principals. These
  are metadata reads, not provisioning or ACL-write permission.
- A daily Cron Trigger in `worker-portal` verifies effective readers and drives
  continuous-rule observation. It uses the existing App key; adopters provide
  no additional credential (§5.4, §6.10.4).
...
- Organization `members: read` where an organization-wide enforcement-scope
  selector or a continuous exemption team must be observed; repository metadata
  is used for live per-repository permission checks
...
### Provisional encryption and key-access map

**Status: direction accepted, details not yet reviewed.** The requirements
review will define the privacy boundary before this section becomes normative.
In particular, the plaintext effective-reader design elsewhere in this draft is
not the intended end state. The current direction is that repository readers
see ciphertext; authority to decrypt, rather than GitHub repository visibility,
controls access to private CLA data. A dedicated CLA organization may remain
optional hardening, but is not required merely to hide plaintext from ordinary
readers or other read-only Apps.

The provisional design uses two independent per-project keys:

- the **records key** encrypts canonical signer records and every private
  derivative, including indexes and exports;
- the **coverage key** encrypts coverage shards, exemptions, operational
  markers, and other private enforcement projections.

The intended access map is:

| Principal | Repository access | Plaintext/key access |
|---|---|---|
| Contributor | None required | May use the portal to read their own status or submit an action; receives no raw key |
| Authorized records reader | None required | May use the portal to read authorized plaintext; receives no raw key |
| Records administrator | Administrative portal and repository access as required | May administer readers and recovery; repository administrators and protected-workflow controllers are necessarily key-capable trust principals |
| Contributing-repository maintainer | Contributing repository only | No records or coverage key |
| `worker-portal` | Writes canonical and coverage repositories | Records key and coverage key, independently wrapped for this service |
| `worker-enforce` | Reads coverage and writes checks | Coverage key only; never the records key |
| Canonical reconciler | Reads canonical and writes private derivatives and coverage | Records key and coverage key through protected Actions secrets |
| Installer | Temporary administrator access while provisioning | Generates both keys transiently, installs the required wrapped/secret copies, and produces recovery material |
| Backup operator | Ciphertext repositories | No key required for ordinary backup |
| Recovery custodian | Offline recovery material | Can recover both keys under the adopter's documented recovery procedure |
| Ordinary repository reader or read-only App | Ciphertext repositories | No decryption key |

The provisional key-distribution shape is:

1. protected canonical-repository Actions secrets hold both keys for the
   reconciler;
2. separately wrapped service copies give `worker-portal` both keys and
   `worker-enforce` only the coverage key;
3. the adopter receives an offline encrypted recovery bundle;
4. ciphertext names a key ID, and rotation retains old decryption keys so the
   append-only history remains readable.

Encryption must cover private derived artifacts as well as canonical events;
one plaintext export or index would defeat the boundary. The secret-bearing
workflow must be strongly protected, and DraCLA Apps that append events must
not be able to modify it. Organization owners, repository administrators,
workflow/secret controllers, DraCLA's hosted services, and recovery custodians
remain trusted because each can obtain or cause use of plaintext or key
material. Exact algorithms, envelopes, key-wrapping ownership, rotation,
recovery, and authorization rules remain open until the requirements are
...
```json
{
  ...
  "type": "acceptance | revocation | agreement_published | agreement_activated
           ...
           | retry_requested | exemption | exemption_source_withdrawn
           | exemption_rule_configured | exemption_rule_withdrawn
           | exemption_materialized
           | records_reader_authorized | records_reader_withdrawn
           | records_reader_rule_configured | records_reader_rule_withdrawn
           | records_acl_observed
           | entity_authorized | entity_deauthorized",
  "recorded_at": "2026-08-17T12:00:00Z",
  "dracla_version": "0.1.0",
  "actor":    { "kind": "github",
                  "github_user_id": 7654321,
...
- **Automation does not impersonate the rule author.** An
  `exemption_materialized` or `records_acl_observed` event instead uses
  ...
  exact GitHub team or ACL observation. The earlier administrative rule event
  retains its human actor and action-time authorization; the later event does
...
overrides/<key>.json
exemptions.json     { "<user_id>": { active: true,
                                     sources: { "<source_id>": {
                                       kind: "individual" | "team_snapshot"
                                             | "continuous_team",
                                       basis, instrument_ref,
                                       rule_event_id, event_id } } } }
records-access.json { status: "ok" | "action_required",
                      generation, observed_event_id,
                      public_reason: "generic_action_required" }
```
...
result for the same operation, and semantic no-ops append nothing. The initial
release adds no replacement abuse policy; a future guard may control transport
abuse only if it preserves immediate valid revocation and fresh re-signing
(`REQ-REV-1`, `REQ-REV-5`).
...
**Records ACL status is operational projection, not signer evidence.** It
contains no reader identity. Exact readers, access paths, authorizations, and
observations remain canonical/private. A non-`ok`, missing, or malformed status
is `action_required` for the whole project and uses only the generic public
wording of `REQ-CHECK-1`.
...
sign / revoke  (Worker):
  0. validate request fields and current tuple state   <- no write yet
     ...
     semantic no-op -> return current result; no write or event
  1. CAS-add idempotency_key -> { subjects: [user_ids] }
     to inflight.json only if no different open operation
     names the same subject                            <- marker OPENS here
  2. re-read and semantically revalidate while holding the subject lock
  3. commit event to canonical              (§5.2)
  4. write users/<shard>.json               (CAS, §5.3)
     write source.json { canonical_sha }               <- effective here
  5. remove idempotency_key from inflight.json (CAS)   <- marker CLOSES here
  6. re-evaluate the originating pull request, if any

enforcer, on every check:
  resolve the coverage branch once -> immutable coverage_sha C
  read records-access.json at C
  if status != ok:
        the project needs administrator action -> action_required; never pass
  read inflight.json at C
  ...
  else read agreements/active.json, exemptions.json, and shards at C,
       and decide
```
...
that commit rather than through a moving branch ref. The marker, records-ACL
status, active agreement, exemptions, and subject rows therefore describe one
internally consistent observation; a check never combines an old marker or ACL
status with newer shards or the reverse. The published check records `C` for
audit and retry.

Concurrency with a coverage mutation has deliberately **unspecified ordering**.
If the mutation's closed projection commit exists before the check selects
`C`, the check observes it. If the check is published before the mutation opens
its marker, it cannot observe it. When the operations overlap — from selection
of `C` through check publication, and from marker open through marker close —
DraCLA promises no winner based on which operation returns first. The check may
report the consistent state at `C`; a later evaluation observes the later
projection. Making completion order decisive would require cross-service
serialization between GitHub repository writes and check publication, which the
requirements do not demand and this design does not claim.
...
drives two observations that require live GitHub authority: effective readers
of canonical and coverage, and membership of continuous exemption teams. The
...
documented pagination bound is indeterminate and fails closed rather than
approving a partial reader or member set.

For records ACLs, the scan uses a `dracla-records` installation token with the
read-only metadata permissions in §4. It compares every effective access path
with canonical individual, snapshot, and continuous-team reader
authorizations. On an unauthorized reader or failed enumeration it first writes
`records-access.json: action_required`, appends a private
`records_acl_observed` audit event, and notifies `worker-enforce` to re-evaluate
the project's open checks. A missed notification is recovered by the enforcer's
daily sweep. Exact reader identities and paths never enter coverage or public
checks. After remediation, a complete successful scan appends the closing
observation, writes `status: ok`, and triggers re-evaluation. An unchanged
healthy scan performs no repository write.
...
enumerate the team. Each join or departure opens the affected subject marker,
appends one automation-attributed materialization or source-withdrawal event,
updates the source fold, and closes the marker. A partial run leaves only the
affected subjects fail-closed and resumes from canonical evidence on the next
observation. Organization membership webhooks are a latency-reducing doorbell;
the daily full observation is the recovery path.
...
does not set the Actions frequency. The portal scan has no Actions-minute cost;
§9.2 includes its bounded Worker request envelope. ACL changes made in the
unsafe order can expose records before this daily detection, which §6.10.4
documents explicitly rather than claiming the scan can retract. §9.2 gives the
remaining cost.
...
pull_request opened / synchronize
  ...
  -> records-access.status at C is not ok                  -> action_required
     (generic public wording; exact access path is private) (§6.10.4)
  -> any subject in inflight.ops at C                      -> in_progress  (§5.4)
  -> any project-wide operation in inflight.ops at C       -> in_progress  (§5.4)
  -> drop subjects exempt in exemptions.json at C — after the
     freshness guard, so a pending exemption change still
     holds its subject                                       (§6.8)
  -> read agreements/active.json at C, including projection metadata
...
team source events materialize into `exemptions.json` (§5.3). The enforcer,
which cannot read canonical, drops a subject when the source fold is non-empty
before evaluating signature coverage. Without that projection artifact the
requirement would be unimplementable, since recorded exemption sources would be
...
| Exempt an individual or selected snapshot member | one `exemption` source per account | `admin` on records | Records the source, basis, and instrument; snapshot is the default team flow |
...
| Authorize or withdraw a records reader or snapshot | `records_reader_authorized` / `records_reader_withdrawn` | `admin` on records | Changes canonical intended-reader policy; it does not mutate GitHub ACLs |
...
members with all or any subset selectable; confirmation appends one exemption
source event per selected account, because canonical permits one logical event
per commit. Later membership changes do nothing. Snapshot is the default and
the UI never silently upgrades it to a continuous rule.
...
administrator acted again. Only teams observable through one of §5.4's bound
App installations are accepted.
...
7. initialize the coverage           its own `coverage` branch: README,
   projection                        source.json, inflight.json,
                                     agreements/active.json, exemptions.json,
                                     records-access.json initially
                                     `action_required`, and 32 empty user shards
...
3. portal: Connect                       the administrator authenticates,
                                         ...
                                         enforcement-scope entry (§7), and that
                                         both Apps are installed on repositories
                                         of the right shape
4. same session: configure               recipient, enforcement scope, privacy
                                         policy, required fields, and initial
                                         reader intent; verify effective access;
                                         publish any agreement versions, then
                                         activate one when it should become
                                         signable (§6.5)
5. portal writes registry entry          last, with one binding record per
                                         enforcement-scope entry, only after the
                                         records ACL is verified `ok` (§7)
```
...
**Reader intent is canonical before GitHub access changes.** A records
administrator may authorize an individual, select all or some current team
members as a snapshot, or explicitly delegate continuously to a team. Snapshot
is the default. A selected subset receives individual GitHub access; granting
the team itself read would expose the records to every current member and is
therefore permitted only when all of them are authorized. Continuous mode warns
that anyone who can change team membership can grant private-records access.

When more people need access than should be owners, a fully authorized team can
receive read on the records repository rather than adding owners or raising the
base permission. Ownership carries administrative control of the whole
organization, which is far more than reading signer data requires, and raising
the base permission re-opens exactly what §6.10.4 exists to close. A read-only
team grants the required access without granting repository administration:
...
Install does not create that team or decide its readers. The safe documented
sequence is mandatory:

1. authorize the individual, snapshot members, or continuous team in DraCLA;
2. grant the corresponding GitHub access; and
3. request immediate verification in the portal.

Doing step 2 first exposes the private records immediately. A later verifier can
stop CLA checks and call for administrator action, but it cannot retract data
already viewed, cloned, or copied. The portal displays that warning before
reader authorization and in the operator README.

Install and the credentialed `worker-portal` verifier (§4, §5.4) enumerate
effective access and refuse or mark `records-access.json: action_required`
until every principal is covered by canonical reader intent. The public check
remains the generic `action required`; an authenticated records administrator
sees the exact owner, team, collaborator, or service-principal access path and
may either authorize it or remove it in GitHub before requesting verification.
An unchanged healthy daily scan writes nothing. A dedicated organization is
the simple default, but an existing organization is conforming when it has base
permission `none` and every unavoidable owner and reader is authorized.
...
`none`, then presents the effective-reader set for authorization during
Connect. There is no visibility-only override:
...
no organization owners, membership, or base permission. Install presents the
owner and every collaborator on both private repositories; Connect must
authorize them and pass the credentialed scan before the registry entry is
written. The bootstrap repositories contain no signer evidence and
`records-access.json` starts `action_required`, so this initial sequencing does
not expose contributor data or make the project routable. The invariant is
identical across personal, dedicated-org, and other-org custody shapes.
...
- `records-access.json` starts fail-closed, and Connect cannot write the
  registry entry until the credentialed reader scan changes it to `ok`
...
revoke append, the portal acquires the subject's in-flight mutation lock and
...
exemptions the source kind, rule reference, basis, and instrument reference.
`records-access.json` adds only a project-wide status and generation; reader
identities and access paths remain canonical/private.
...
idempotency makes an exact retry write-free, and semantic no-ops append nothing.
Request-flooding risk #7 is monitored and handled operationally rather than by
collecting another identifier (§5.3, §8.4).
...
- **Subrequests — bounded for an ordinary pull-request check.** It costs route
  ...
  fixed coverage reads for `records-access.json`, `inflight.json`,
  `exemptions.json`, and `agreements/active.json` (4) + subject coverage shards
  (1–32) + check run write (1), or 11–44 against a cap of 50. The 32-shard Free layout (D9)
...
1. admin runs:  uvx dracla install github.org=<org>-cla   (own credentials)
     ...
     -> require base permission `none`, enumerate every effective reader, and
        present them for later confirmation; the CLI grants no authorization
...
4. admin connects in the portal: recipient, agreements, enforcement scope,
   policy text, initial reader intent
     -> each coverage- or evidence-affecting change becomes an event with an
        actor and authorization evidence; each initial effective reader is
        authorized explicitly or by a continuous team rule, then the
        credentialed scan must pass and set `records-access.json: ok`; one
        agreement version is activated explicitly before signing is enabled
...
account it enumerates the owner and collaborators. Install presents that set
but cannot authorize it: Connect records the initial reader intent as
administrator-attributed canonical events and keeps the seeded ACL status
fail-closed until a complete scan passes. Private visibility or base permission
alone never passes (§6.10.4). After connect, the daily `worker-portal` verifier uses the
`dracla-records` App's repository `administration: read` and organization
`members: read` metadata permissions to repeat that enumeration. Unauthorized
or indeterminate access writes the project-wide records-ACL blocker and causes
the enforcer to re-evaluate affected checks as generic `action required`, as
`REQ-REC-1` and `REQ-VERIFY-2` require.
...
| Daily portal verification | 2 Worker requests per project | conservative allowance for one bounded fan-out delivery and one project ACL/continuous-rule scan; GitHub API calls are subrequests inside the project invocation |
...
including the daily portal-verification allowance)

| Projects | Requests/day | % of Free | % of Paid | Known request-path CPU s/day |
|---|---|---|---|---|
| 10 | 780 | 0.8% | 0.2% | 1.0 + scan probe |
| 50 | 3,900 | 3.9% | 1.2% | 4.8 + scan probe |
| 100 | 7,800 | 7.8% | 2.3% | 9.6 + scan probe |
| 250 | 19,500 | 19.5% | 5.9% | 24.0 + scan probe |

Cloudflare Free saturates at roughly **1,280 projects**; about 500 if every adopter is
...
123 Worker requests/day. The 250-project envelope produces 30,750 requests/day
(30.75% of Free), and the Workers request ceiling moves to roughly 813 projects.
This sensitivity changes request volume only; maximum-batch CPU and subrequests
remain the separate A2 release probe.

The daily ACL/continuous-rule project scan requires its own real Free-tier CPU
probe at the maximum supported owner, team, collaborator, service-principal,
and team-member page count. I/O wait is not CPU, but signature verification,
JSON parsing, authorization-source comparison, and fan-out bookkeeping are.
The initial release may keep a bounded pagination profile that fails closed; it
may not claim the 250-project envelope until one scan invocation stays below
10 ms and its fan-out stays below 50 subrequests.
...
At the Workers-request saturation point of roughly 1,280 projects, this bound
is about 57,600 Durable Object requests/day, 7,200 GB-s/day at the
one-second duration budget, and about 154 MB of empty SQLite object
...
**Subject to A2's portal-scan CPU probe, the binding modelled constraint remains
...
the credentialed periodic ACL check, and uncapped successful revocation and
re-signing.
...
| `REQ-CHECK-2` | `Co-authored-by` trailers no longer determine a public check result, and are surfaced to authorized viewers instead; exemptions use individual, snapshot, and explicit continuous sources whose effective result is their union | §5.3, §6.3.1, §6.8, D14 |
| `REQ-REC-1` | Reader intent is canonical before GitHub access; the credentialed daily portal verifier fails closed on unauthorized or indeterminate effective access and documents the irreversible unsafe-order exposure | §4, §5.4, §6.10.4 |
| `REQ-REV-1`, `REQ-REV-5` | Successful acceptance and revocation are uncapped; idempotent retries and semantic no-ops remain write-free | §5.3, §5.4, §6.2 |
...
Resolved. The baseline is **Locked at revision 11** and this document is
...
evidence. Revision 11 adds individual, snapshot, and explicit continuous
exemption and reader sources; a credentialed periodic records-ACL verifier and
safe authorize-before-grant workflow; and uncapped successful acceptance and
revocation. Requirements section 20 records each change with its rationale,
...
- **A2 — Subrequests bounded; maximum-shard, merge-group, portal-scan, and Durable Objects measurement required before release.**
  An ordinary pull-request check uses at most 44 subrequests, including route
  and gate lookup, the immutable coverage-ref snapshot, records-ACL status, and all 32 Free-profile
  ...
  distribution. Four things remain: measure all 32 shards at the maximum
  supported shard size; confirm ordinary and maximum-batch merge-group CPU-time
  percentiles on a real Cloudflare account; measure the maximum supported
  portal ACL/team scan below 10 ms and 50 subrequests; and measure Durable
  Object latency and duration against A3's one-second budget rather than
  extrapolating them from a development machine. The shared daily request
...
- **A3 — Capacity assumptions complete; model update and release measurements pending.**
  ...
  saturates first, near 1,280 projects in the normal no-rebuild case and near
  813 under the ten-delivery merge sensitivity. Actions minutes in each
...
| R7 | ~~One busy adopter consumes the shared hosted ceiling~~ **Downgraded** by the A3 model: the normal no-rebuild case saturates near 1,280 projects, the ten-delivery merge rebuild sensitivity near 813, and the sampled `cli/cli` case near 500 (§9.2) | No persistent per-project counter is added; paid capacity and monitoring are the operational guard |
