...
3. portal: Connect                       the administrator authenticates,
                                         DraCLA verifies they administer BOTH
                                         the org the slug names and the one
                                         holding the records (§7), and that both
                                         Apps are installed on repositories of
                                         the right shape, then writes the entry
...
`acme`, whatever the records live in. Every org and repository in `scope` must be
covered by the enforcer installation, and claims are first-come and never
