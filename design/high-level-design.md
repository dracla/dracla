...
Status: Draft
Date: 21 August 2026
Requirements baseline: `design/requirements.md` (Locked, revision 4, 21 August 2026)
...
```json
{
  ...
  "type": "acceptance | revocation | agreement_published | agreement_activated
           | project_connected | config_updated | enforcement_scope_changed | override
  ...
  "authorization": { "resource": "acme/acme-cla-records",
                     "permission": "admin", "checked_at": "…" },
...
- **`authorization` accompanies every administrative event.** It records the
  resource and live permission check from `REQ-SEC-6`; the stable actor ID and
  login snapshot remain in `actor`.
- **`configuration` is event-type-specific.** An
  `enforcement_scope_changed` event carries the full effective enforcement
  scope; an acceptance omits it. Enforcement configuration is never copied
  into signer evidence.
...
On replay, the server recomputes the key from the authenticated actor,
operation target, payload, prior event, and form nonce. Finding the same key
with different inputs is a conflict; it is never accepted as an idempotent
success.
...
- **Step 5 must re-validate, not just re-parent.** A revocation of acceptance
  ...
  is already an idempotent no-op, or fails as a conflict. It never changes the
  revocation to target `E2` and never appends an operation whose meaning
  depends on retry order.
...
agreement overwrite the first. Inactive versions are never signable, so there
is no pending-version state.

**The edge performs one bounded version test.** A non-revoked acceptance row
provides coverage only when its `version` is in
...
removed by an earlier superseding activation (§6.5). Enforcement scope is
resolved by project routing before any subject row is read; it is not signer
evidence and does not live in the row.
...
no project-wide revoke-all action. The agreement-specific confirmation screen states that
...
the acceptance it revokes — plus the agreement identifier, as `REQ-REV-3`
requires — and only that agreement's projection row flips to `revoked`.
Re-signing appends a new acceptance and never mutates the revoked event.
...
**Enforcement-scope changes are not activations.** They append an attributable
`enforcement_scope_changed` event and update project routing after the required
owner authorization (§7). They decide where DraCLA checks run; they do not
change agreement version currency, alter a legal grant, or require re-consent.
...
|---|---|---|---|
| Connect a project or explicit successor | `project_connected` | `admin` on records and every affected installation | Records immutable project and recipient identity, plus `successor_of` when applicable |
...
| Change enforcement scope | `enforcement_scope_changed` | `admin` on each repository added, or owner of an organization-wide selector | Changes routing only; removals require records `admin` and no added-owner consent (§7) |
...
event records stable numeric actor ID, login snapshot, resource, observed
permission, and check time. Authorization is rechecked when the action occurs;
a session or permission on another project is not enough. Install and credential
rotation use the administrator's own credentials and require `admin` on every
affected repository or App installation (§6.10).
...
permission or organization-membership endpoint and returns only the observed
permission and check time. No user or installation token crosses the boundary;
the records event stores the result as authorization evidence.
...
private repository names, enforcement scope, and installation ids. `CODEOWNERS` and required
review apply to it.
...
**Enforcement scope is bound by the same authority** — required by
`REQ-CONFIG-5`. An entry is `owner/name` or
...
entry itself**: every enforcement-scope entry carries `bound_by` — the numeric
user ID of the administrator whose owner-administration was verified — with `bound_at`
...
record no consent, because narrowing needs none. Both additions and removals
append an `enforcement_scope_changed` event to canonical before routing changes,
so current enforcement scope is replayable under `REQ-REC-8`. An `owner/*` entry is
standing consent for present and future repositories — the same semantics as installing an App on all
...
The cost is deliberate and recorded: a federated project whose enforcement-scope owners
...
- The KV write is driven from a verified registry commit, never directly from
  ...
  signature, discards the payload, and re-reads the registry from the
  repository over the GitHub API — the webhook is a doorbell, the repository
  is the source, and the registry is small enough to re-derive whole, so no
  change detection is needed — then signs each entry with the key in its
  secrets and writes KV.
- The reconcile schedule drives the same handler over the full registry,
  re-deriving KV and repairing drift — the same rule the coverage projection
  follows (`REQ-REC-6`).
...
- The installation token used must belong to **one of** that project's
  ...
  different installations and may live under different owners; §6.10.4 checks
  the effective readers of every custody shape, so this is the
...
- A contributing repo in no project's enforcement scope receives no check.
- **Every authorization decision is scoped to the resolved project**, and no
  ...
  project per request.
- Enforcement scopes across projects must not overlap. A contributing repository resolving
...
| KV entry signing key | Worker secrets | Add the new key beside the retiring one and verify against either; the reconcile schedule's full re-derivation re-signs every entry, then the retiring key is dropped (§7) | Operator departure rotates it | With a KV write path, forge routing entries |
...
| Head-identity log signing key | Adopter canonical Actions secrets | Add a successor and publish its verification key before signing new records with it; retain predecessor verification keys for old log entries | Adopter **maintainer** departure rotates it | Forge new head-log entries for one project, but not rewrite canonical git history or old signatures |
...
signature verification at the very edge, and Workers Paid for the shared hosted
account. The §9 route split isolates secrets and request handlers, **not** the
account-wide request quota; quota isolation would require separate Cloudflare
accounts. Risks R7 and R9.
...
rate limiting and the hosted Paid-plan budget, not by the two Workers' route
split, because both share one account quota (§9).
...
| `REQ-CONFIG-3` | **Constraint acknowledged** — cross-organization enforcement scope is definable, but in v1 each entry must be bound by an administrator of its owner (`REQ-CONFIG-5`, §7); the agreement, not this configuration, defines legal scope |
...
Resolved. The baseline is **Locked at revision 4** and this document is
...
- **A4 — Merge queue on GitHub Free. CLOSED, verified 18 August 2026.**
  ...
  rather than assumed.
- **A5 — Federated enforcement scope. OPEN by design.** A project whose enforcement-scope owners share
...
| R9 | `dracla-enforcer` is a public App, so an outsider can exhaust the shared per-account budget and halt checks, signing, and revocation for every adopter | WAF and rate limiting ahead of the routes; signature verification at the edge; Workers Paid for the shared account. The two-Worker split does not isolate an account-wide quota (§9) |
