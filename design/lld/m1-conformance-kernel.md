...
| `enforcement_scope_activated` | `derived_index` |
...
`enforcement_scope_activated` is index-only for that reason: `enforcement_scope`
is an index row member and the status-detail schema has no scope field, so
advancing `status_detail` would fail exact-status reads closed during the
project-wide index publication without any status-detail byte having changed.
...
- `validate_derived_projection(projection, *, generations, encrypted_shards,
  profile) -> None`; and
- `resolve_read_status(row, active_agreement, operation_state)
  -> ResolvedStatus`.
...
- `exemption_sources`, the subject-scoped provenance §4 assigns to the records
  ...
  "rule_event_id", "sequence"}`, with every member always present; and
...
provenance. A full materialization assigns sequences in ancestry order and an
incremental addition takes the current maximum plus one, so both produce the
same values. Withdrawal removes an entry without renumbering; gaps are expected
and the maximum remains well defined. Timestamps are never used for this,
because ancestry is the only ordering this design recognizes.
...
**Rows store durable facts; `status` and `reason_code` are derived at read.**
A stored `status` would have to change whenever an activation changed which
...
that the row rests on. Those values never change when another
subject's agreement is activated.
...
| `no_acceptance_recorded` | event that removed the last basis | `null` | `null` | `null` |
...
agreement entry carries exactly `version`, `enforcement_scope`, `accepted_at`,
`revoked_at`, and the same `basis` object — §6.6's row fields minus the two
that are the container keys, with `status` derived at read rather than stored.
`reason_code` and `exemption_sources` are deliberately absent: §8.4 keeps the
index to what the mandated filters need, and the exact reason is status-detail
data.
...
`enforcement_scope` is the project's effective scope at materialization, copied
into each entry so a bounded page can satisfy §6.6's mandated filters without
another read; it is project-level truth resolved by routing, never per-subject
truth, and a materialization refreshes it for every row it writes.
...
denied for the same reason, an exemption layered over a
revoked row and then withdrawn to prove the revocation is still served, an index
page deriving `exempt` from its own shard alone, a subject exempted before any
agreement is published reading `exempt` for the agreement after the first
activation through a synthesized row,
exact/private data separation, canonical/derived generation agreement, envelope
digest tables, affected-only deltas, incomplete bulk publication, and every
profile limit at `limit-1`, `limit`, and `limit+1`, including the retry and
subrequest ceilings. A profile case rejects an unknown root member, a missing
one, a `max_shard_ciphertext_bytes` object carrying a coverage member, and a
`max_subrequests_per_request` of 50 or more, a `standing_rules` map exceeding
`max_continuous_exemption_rules`, and a state file exceeding
`max_state_ciphertext_bytes`. An exemption-provenance case gives one subject
several active sources and proves both runtimes select the same latest source by
`sequence`, that a withdrawal leaving gaps keeps the maximum well defined, and
that a full materialization and an incremental addition assign identical
sequences. A `standing_rules` byte case fixes
the map encoding with the full §5.1 team object, and a digest case rejects a
shard digest encoded as bare hex or base64url rather than the
`sha256:`-prefixed form. A dedicated case drives a coverage shard past the
largest records-derived limit and proves the acceptance and revocation still
complete, the coverage write is not rejected, and only the affected private
derived class fails closed.
