...
Two edge-executed contracts are deliberately outside M1's TypeScript scope
because their drivers do not exist yet, and both are named here so neither is
discovered late:
...
- the §5.4 canonical write path. The portal Worker is the canonical writer and
  ...
  prepared cell, so M5 needs edge-executable event schema, identity,
  semantic-relation, and side-artifact validation. M1-5 and M1-9 author those
  contracts and their vectors in Python; M5 implements the edge subset it
  executes against exactly those vectors.
- the §6.10.4 records-derived read path, whose derived-state and
  reader-authority validation M1-11 authors and M5 executes.

In both cases the later milestone adds its TypeScript implementation against
this manifest and the M1-owned vectors for that contract. It does not start a
second corpus, re-specify a contract, or let one runtime become the other's
oracle. Moving either implementation earlier would widen M1 closure and is a
roadmap decision, not an implementer's choice.
