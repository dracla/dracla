...
| M1-13 — TypeScript byte foundations and artifact envelopes | Planned | M1-12 | JCS, base64url, identity, AAD, envelope, profile, projection decoders |
...
| `projections.ts` | M1-13 | Read-only decoders for every projection plaintext |
...
| `exemption`, `exemption_snapshot`, `exemption_source_withdrawn`, `exemption_materialized` | `derived_index`, `status_detail` |
| `exemption_rule_withdrawn` | `status_detail`, and `derived_index` only if a subject's effective exemption flips |
...
which is §5.2's effect-based rule; the table states the effect each type has.

`exemption_rule_withdrawn` shows why the rule is effect-based rather than
type-based: withdrawing a rule that never materialized a subject, or whose
subjects all remain exempt through another source, changes no index byte. It
always advances `status_detail`, because `standing_rules` and the provenance
change, but it advances `derived_index` only when a subject's effective
exemption actually flips. The Worker decides that from the prior exemption union
it already holds as a precondition, so no extra read is needed.

One other case cannot be decided by effect at write time, and it is stated
rather than left to the table. A repeated reader withdrawal is the case, and the
signal is a deletion rather than a record: a withdrawal removes the source
record, so an already-withdrawn source is represented by its *absence* from
`reader_authority`.

Against a current class that absence is trustworthy, so the edge knows the fold
changes nothing and declares no class affected. Against a stale class it is not:
the class may still show the source present precisely because it has not
caught up with the withdrawal, and the edge cannot distinguish that from a
source that is genuinely still active. It therefore declares `reader_authority`
affected even though the fold changes nothing. The ambiguity exists only when
the class is stale.

Declaring it affected is also the safe direction. The generation advances while
the derived class is already behind and its update was skipped, so reads on that
class keep failing closed until the reconciler repairs them, which is exactly
the stale-path outcome already specified. Replay accepts the declared package
rather than recomputing affectedness for this case.
...
oversized. A `generation` binding there would reject the withdrawal before
canonical append in exactly the situation the rule exists to protect. Removing
state cannot make a stale class worse, and canonical replay remains the
authority on the resulting union. When the class is already stale the derived
update is skipped rather than attempted, so the append succeeds while that
class stays behind until repair; M1-18 states that path. A withdrawal of an
already-withdrawn source always appends and is folded by M1-7 as an idempotent
no-op. It never takes the §5.4 terminal-no-op path, because that path needs
`terminal_noop_event_id` and the withdrawing event's ID is not obtainable: it
derives from a random operation nonce, and the reader-authority record that
might have carried it is deleted when its last subject is withdrawn or its rule
is retired. That holds whether or not the class is current, so there is one
behaviour rather than two.

§5.4 permits this. It sets `terminal_noop_event_id` when the desired effect is
already true *and* the establishing event is known, and `null` otherwise; a
`null` field is an ordinary write. The cost is one redundant canonical event,
which §5.3 already tolerates — it caps neither acceptance nor revocation and
adds no abuse guard — and the alternative would be retaining withdrawal
provenance solely to satisfy a no-op path.

A continuous-team departure is the third removal form —
`records_reader_materialized` with `result: withdraw` — and it is deliberately
*not* shrink-only. Its nonce derives from `prior_materialization_event_id`,
which the Worker reads from the reader-authority `materialization_cursor`; a
stale class is stale precisely because recent reader events are missing from it,
so that prior may be behind and appending under it would mint a wrong identity.
The observation is therefore deferred until repair rather than forced through.

Deferring it is safe, which is why §9's availability rule does not reach here.
That rule is about administrator-initiated withdrawals, which are the two
shrink-only forms above and which remove state directly. For a departure,
§6.10.6 guarantees that "a missed or failed departure observation still makes
the next live membership check deny plaintext before canonical withdrawal", so
confidentiality never waits on the automation landing. What is delayed is the
canonical record, not the denial.

Exemption withdrawal is not shrink-only in this sense and keeps its
`generation`-bound union input, because that input feeds a coverage recompute
that must be correct rather than merely permitted; a stale `status_detail` class
fails it closed, which is the documented derived-overflow behavior for that
class.

**Declaring a requirement and executing it are separate.**
`required_preconditions` returns descriptors — which relations must hold, which
artifact or event path decides each, and under which binding mode — and
validates those relations over already-validated models it is given. It does
not fetch, decrypt, or decode a projection artifact, because the closed models
for coverage and records-derived plaintexts are authored by M1-10 and M1-11,
...
and bound by head or generation before this contract compares anything.
...
A shrink-only case proves a `records_reader_withdrawn` and a rule withdrawal
both succeed while the `reader_authority` class is stale and while it is
oversized, and that withdrawing an already-withdrawn source appends and folds
to the same union under replay rather than failing it, against a current class
and a stale one alike — declaring no class affected in the first case and
`reader_authority` affected in the second. A departure case proves a
`records_reader_materialized` withdraw is deferred rather than appended while
the class is stale, and that the live membership recheck denies plaintext in
the meantime.
...
**A repeat source or rule withdrawal folds as a no-op, not corruption.** A
withdrawal naming a source that the fold has already removed is valid and
changes nothing: the union is already without it. Rejecting it would make the
shrink-only append path of M1-5 unsafe, because a Worker facing a stale
`reader_authority` class cannot discover the prior withdrawal — the withdrawing
event's ID derives from a random operation nonce, so its path is not derivable,
and the derived class it would otherwise consult is exactly the one that is out
of date. Treating the repeat as an idempotent fold keeps §9's withdrawal
availability and replay determinism compatible. Nothing in the locked design
classifies a repeat withdrawal as corruption; only an unknown or wrong-project
source is invalid.

Negative sequences include two scope terminals, mismatched repeated scope,
a source or rule that never existed or belongs to another project, stale
materialization predecessor, wrong team/subject/evidence, partial batch
identity, boolean-overwrite behavior, wrong override grant relation, and any
forbidden post-successor event. A repeat withdrawal is asserted to fold as a
valid no-op rather than appearing among them.
...
coverage decision, exemption byte, or subject shard, and needs no union input.
That distinction matters for availability: §9 requires reader withdrawals to
stay available even when additions or derived shards exceed profile limits, so a
stale or overflowed `reader_authority` class must never block one. Requiring
reader provenance here would have done exactly that.

Carrying no union input is not the same as making no coverage write. §5.4 step 7
advances `source.enc.json` after every canonical append, and the derived read
binding requires its `canonical_sha` to equal the current events head, so an
event that skipped that write would strand the binding and fail every later
portal read closed. Every event that appends therefore includes the
source-file advancement — the canonical commit and the replayed lifecycle fold —
in its coverage delta, reader events included. What varies by event type is
which decision, exemption, and shard bytes change alongside it.
...
A status-detail shard maps `github_user_id` to a subject record with exactly
six members — `login_snapshot`, `login_as_of`, `exemption_sources`,
`exemption_sequence_next`, `materialization_cursor`, and `agreements` — grouped
below by the scope of the evidence they hold:
...
- `exemption_sources`, the subject-scoped provenance §4 assigns to the records
  ...
  `exemption_sequence_next` counter accompanies the map, as does a sibling
  `materialization_cursor` map keyed by `rule_event_id` holding the latest
  `exemption_materialized` event ID for this subject under that rule;
...
|---|---|---|---|
| `bot` | `null` | each a string or `null` | `null` |
...
`materialization_cursor` exists because §5.1 derives an automation nonce from
`prior_materialization_event_id`, which is what makes an add → withdraw → add
sequence produce three distinct identities rather than repeating the first. Each
of those three transitions is itself a materialization event — §5.1 gives
`exemption_materialized` and `records_reader_materialized` a `result` of `add`
or `withdraw` — so the cursor records the withdraw transition too, and the final
add references it. A
withdrawal deletes the source entry, and the Worker does not fold canonical
history, so without a surviving cursor the second add would pass `null` and
recreate the first add's identity while a full replay expected it to reference
the withdrawal. The cursor is written on every materialization and survives
removal; only a rule withdrawal, which retires the rule entirely, drops it.

The two cursors are bounded differently, and the reader one needs its own limit.
The exemption cursor is keyed by `rule_event_id`, so it holds at most one entry
per active rule and `max_continuous_exemption_rules` bounds it directly. The
reader cursor is keyed by subject and its entries outlive removal, so it
accumulates one entry per distinct account ever materialized under that source —
cumulative churn, not concurrent membership, which is all
`max_reader_source_members` bounds. It therefore has a dedicated
`max_reader_cursor_entries` limit.

Pruning is not an alternative: an entry is exactly what a later re-add needs to
avoid recreating the first add's identity, so dropping one reintroduces the
collision it exists to prevent. Reaching the limit fails the next
materialization closed rather than dropping an entry, and the operator remedy is
to withdraw the rule and configure a new one, which retires the source record
and its cursor together and starts a fresh identity space.
...
for `bot`" — that makes each field *optional* for a bot source, not required to
be absent, so a bot exemption recorded with a basis or instrument reference
keeps it. Every other kind requires both as strings. `rule_event_id` lets a
rule withdrawal find every entry it materialized, which is the identity
...
which is what the status and reason are for. The acceptance timestamp is
retained here, unlike the revoked case, because the acceptance did happen and
only its version stopped being current.
...
- `team`: `null` for `individual`, and otherwise §5.1's complete team object
  `{"organization_id", "team_id", "slug_snapshot"}` for `snapshot` and
  `continuous_team`. The `slug_snapshot` is retained, not just the numeric IDs,
  because the daily observer constructs `records_reader_materialized` whose
  `team` must match the standing rule, and it cannot fold history to recover the
  configured snapshot; substituting the current GitHub slug would diverge after
  a team rename;
- `materialization_cursor`: a map keyed by the decimal `github_user_id` string
  whose value is the latest `records_reader_materialized` event ID for that
  subject under this source. Entries survive a subject's removal from
  `subjects`; and
...
max_reader_cursor_entries    integer
...
invocation; `max_continuous_reader_rules` bounds active continuous reader
rules; `max_continuous_exemption_rules` bounds active continuous exemption
rules, whose `standing_rules` entries would otherwise grow without limit inside
a file every derived read must fetch and validate; `max_reader_cursor_entries`
bounds one reader source's `materialization_cursor`, which accumulates across
membership churn rather than tracking concurrent membership; and
`max_state_ciphertext_bytes` bounds `derived/state.enc.json` itself, which the
per-class shard limits do not cover. An additional rule past either count is
rejected before its canonical event is appended, exactly as §6.10.4 does for
the eleventh reader rule. `max_shard_ciphertext_bytes` has no coverage member,
which is what makes a coverage size limit unrepresentable rather than merely
discouraged.
...
- members per reader source, and cursor entries per reader source;
...
32 digest keys per class with unchanged shards carried forward. A
standing-rules case configures a continuous exemption rule with no members,
proves the Worker can enumerate it from `derived/state.enc.json` alone without
folding history, materializes and then removes every member, and proves the
rule is still enumerable so the observation can restart; withdrawing the rule
removes the entry. A named-subject case proves the view renders from the stored
`login_snapshot` and `login_as_of` alone, with no index read and no
current-login substitution. A login-advance case replays several events for one
subject with differing logins and proves both sources advance the pair — the
subject acting, and an administrator event carrying that subject's `subject`
object — that the latest by ancestry wins, that an administrator's own actor
login never lands on the subject they name, that each member of a `subjects`
set advances from its own entry, and that both members stay `null` only until
some event carries that subject's login. Exemption-source byte cases fix all
four `source_kind` encodings with their exact null patterns and reject an
`individual` entry with a `team`, an `individual` entry with a null `basis`,
and a `continuous_team` entry with a null `rule_event_id`. A bot case covers
all four combinations of present and absent `basis` and `instrument_ref`, each
accepted and round-tripped. A snapshot case authorizes several subjects in one
event and proves each is independently withdrawable: withdrawing one leaves the
others authorized under the same `source_id`, and withdrawing the last removes
the source record. Other cases prove a withdrawn reader is absent rather than
...
activation, an `exemption_rule_withdrawn` that flips no subject's effective
exemption advancing `status_detail` alone while a flipping one advances both, a
correction chain proving A2 replaces A1 in the single row without producing a
`superseded` entry, an activation that changes what every subject reads while
rewriting no index or status shard and advancing no class generation, a read
against a coverage head predating that activation failing closed on the
`canonical_sha` binding rather than reporting the former version, a rolled-back
coverage ref with an idle fence and valid derived generations denied for the
same reason, an exemption layered over a revoked row and then withdrawn to
prove the revocation is still served, an index page deriving `exempt` from its
own shard alone, a subject exempted before any agreement is published reading
`exempt` for the agreement after the first activation through a synthesized
row, exact/private data separation, canonical/derived generation agreement,
envelope digest tables, affected-only deltas, incomplete bulk publication, and
every profile limit at `limit-1`, `limit`, and `limit+1`, including the retry
and subrequest ceilings. A profile case rejects an unknown root member, a
missing one, a `max_shard_ciphertext_bytes` object carrying a coverage member,
and a `max_subrequests_per_request` of 50 or more, a `standing_rules` map
exceeding `max_continuous_exemption_rules`, a reader `materialization_cursor`
exceeding `max_reader_cursor_entries` after repeated membership churn, and a
state file exceeding `max_state_ciphertext_bytes`. An exemption-provenance case
gives one subject several active sources and proves both runtimes select the
same latest source by `sequence`, that a withdrawal leaving gaps keeps the
selection well defined, and that a full materialization and an incremental
addition assign identical sequences. An add/withdraw/add lifecycle empties the
map entirely and proves the next addition takes its sequence from
`exemption_sequence_next` rather than restarting at zero, and that its
automation nonce derives from the prior materialization recorded in
`materialization_cursor` rather than from `null`, so all three transitions have
distinct identities. The equivalent reader lifecycle proves the same for a
continuous team, and a rename case proves a materialization uses the rule's
stored `slug_snapshot` rather than the current GitHub slug. A subject exempted
...
`artifacts.ts`, `envelope.ts`, `releaseProfile.ts`, and `projections.ts`.
Implement RFC 8785 encode/decode with the shared safe-number model, unpadded
base64url, the complete §4 identity table with dynamic path tokens, artifact
AAD, A256GCM encrypt/decrypt over Web Crypto, and closed release-profile parsing
with size enforcement.

`projections.ts` adds read-only decoders and validators for every projection
plaintext this milestone closes: coverage source, active agreement, exemption
fold, user shard, marker, fence, derived state, dashboard index, status detail,
and reader authority. They decode and validate; they compute no update and no
decision.

They land here rather than with the slices that write them because M1-15's
precondition checks must authenticate active-agreement, coverage, status-detail
provenance, and reader-authority evidence, and M1-15 precedes both incremental
slices. Leaving the decoders until M1-17 and M1-18 would force M1-15 either to
trust already-decoded caller data or to duplicate a schema implementation, and
both break the single-contract boundary. Decoding is a byte contract, which is
this slice's remit; M1-17 and M1-18 add the update and decision logic on top.
...
No keyrings, events, forms, routing, or Worker wiring. No projection *update* or
*decision* logic: this slice decodes and validates projection plaintexts, while
M1-17 and M1-18 own the incremental writes and the subject decision.
...
A repeated *reader* withdrawal is deliberately not in that set. It appends and
M1-7 folds it as an idempotent no-op, whether or not the class is current: the
edge never manufactures a `terminal_noop_event_id` it cannot obtain, and the
withdrawing event's ID is not obtainable from any retained state. Rejecting it
here would demand evidence the Worker cannot bound and put this slice in direct
conflict with M1-5 on a shared case.
...
or unreadable one rejected, including a second scope terminal, an unpublished
activation, an exemption source outside the subject's union, and a
succeeded-project action. A repeated reader withdrawal is asserted to append in
both runtimes and fold to the same union under replay, against a current class
and a stale one alike — never a rejection and never a manufactured terminal
identity.
...
no such input and change no coverage decision, exemption byte, or subject shard,
so a stale `reader_authority` class cannot block a withdrawal — but they still
advance `source.enc.json` like every other appending event, because §5.4 step 7
requires it and the read binding depends on it.
...
Every M1-12 triple round-trips: TypeScript applies the input to the
before-state and reproduces the after-state byte for byte. Cases cover all 32
shard boundaries, multiple agreement versions, revocation and re-signing,
superseding and non-superseding activation, exemptions, overrides, successor
state, and subject-scoped versus project-wide markers. A multi-source exemption
case withdraws one of several sources and proves the subject stays exempt, then
withdraws the last and proves the entry is removed. A reader-authorization case
proves the coverage delta advances `source.enc.json` while changing no
decision, exemption, or subject shard, and that a portal read immediately
afterwards satisfies the `canonical_sha` binding rather than failing closed. A
crash fixture completes the coverage update with the `derived` branch still
unwritten and proves the result is correct, so coverage never waits on private
derivation. A missing or stale union input is rejected rather than guessed.
Negative cases cover wrong format or count, malformed before-state, invalid
override key, forbidden private fields, and every fail-closed marker and fence
inconsistency, including a `success_reserved` fence with a non-empty in-flight
map.
...
**A reader mutation onto an already-stale class does not publish.** The
shrink-only precondition lets a withdrawal append when `reader_authority` is
stale, which is what §9 requires, but the Worker still cannot turn that append
into a generation-completing update: intervening reader events are missing from
the out-of-date shard, so applying the delta and publishing the current
generation would assert a state that was never materialized. Requiring a current
before-state instead would reimpose exactly the block the shrink-only path
removes.

The update is therefore skipped, not attempted. The canonical event stands, the
class generation stays behind the canonical one, and private reads on that class
keep failing closed until the reconciler replays and repairs it — the same
posture §6.6 already defines for a failed or incomplete derived update. This is
the one reader path that does not complete its class, chosen deliberately:
availability of the withdrawal is a locked requirement, while derived freshness
is explicitly allowed to lag behind canonical truth.
...
fields in the index, and incomplete fan-out. A stale-class case appends a reader
withdrawal while `reader_authority` is behind and proves the derived update is
skipped, the class generation stays behind, and reads on that class keep failing
closed until repair, while the canonical event stands. Cross-branch fixtures run
both runtimes through the crash windows above and require identical served
statuses and identical fail-closed decisions.
