...
keeps it. Every other kind requires both as strings. `rule_event_id` lets a rule withdrawal find every entry it
...
exact null patterns and reject an `individual` entry with a `team`, an
`individual` entry with a null `basis`, and a `continuous_team` entry with a
null `rule_event_id`. A bot case covers all four combinations of present and
absent `basis` and `instrument_ref`, each accepted and round-tripped. A snapshot case authorizes several subjects in one event and
