...
confirmed canonical head — the active-agreement file, coverage shards, exemption
fold, project lifecycle in `source.enc.json`, reader-authority shards,
status-detail provenance, or a direct read of a derived event path such as the
two scope-terminal children of §5.1. Each requirement names the artifact, its §4
