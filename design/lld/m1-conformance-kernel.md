...
`sequence`, that a withdrawal leaving gaps keeps the selection well defined, and
that a full materialization and an incremental addition assign identical
sequences. An add/withdraw/add lifecycle empties the map entirely and proves the
next addition takes its sequence from `exemption_sequence_next` rather than
restarting at zero. A scope case proves a scope change rewrites no index or
status shard and that a synthesized row is served with the routing-resolved
scope. A `standing_rules` byte case fixes
