...
removal; only a rule withdrawal, which retires the rule entirely, drops it. The
exemption cursor is keyed by rule and therefore bounded by
`max_continuous_exemption_rules`; the reader cursor is keyed by subject and
bounded by `max_reader_source_members`, with an over-limit input rejected before
append like every other bounded administrative input.
