...
rather than left to the table. A repeated reader withdrawal appended against a
*stale* `reader_authority` class declares that class affected even though the
fold changes nothing, because the edge provably cannot tell: the class that
would reveal the prior withdrawal is the stale one. Against a current class the
absence of the record is visible, so the edge knows the fold changes nothing and
declares no class affected. The ambiguity exists only when the class is
stale.
...
oversized, and that withdrawing an already-withdrawn source appends and folds to
the same union under replay rather than failing it, against a current class and
a stale one alike — declaring no class affected in the first case and
`reader_authority` affected in the second. A departure case proves a `records_reader_materialized` withdraw is
