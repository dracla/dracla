...
current head has already passed; all fail closed.
...
when the artifact already exists, a snapshot whose recomputed SHA-256
...
affected subjects' remaining source union as an explicit authenticated input,
not as something recomputed from coverage. Its source is the records-side
provenance already required to exist — the `exemption_sources` of the subject's
status-detail row, and the equivalent reader-source records — supplied as a
precondition under the `generation` binding mode. Only `worker-portal` writes
coverage and it holds both keys, so this needs no new capability;
`worker-enforce` reads the derived boolean and never performs these
transitions. Materializing the union into coverage instead is forbidden: it
would put provenance inside the enforcement capability.
...
A status-detail shard maps `github_user_id` to a per-`agreement_id` entry
carrying the `status` enum, a `reason_code` from the closed vocabulary below,
the deciding `event_id`, `version`, `accepted_at` and `revoked_at`
as timestamps or `null`, and `exemption_sources` — the provenance §4 assigns to
the records key, each entry naming its `source_event_id`, `source_kind`,
`basis`, and `instrument_ref`. The `reason_code` vocabulary is exactly:

| `reason_code` | `status` | Generic coverage code |
|---|---|---|
| `accepted_current_version` | `current` | `covered` |
| `exempt_source_active` | `exempt` | `covered` |
| `revoked_by_contributor` | `revoked` | `uncovered` |
| `no_acceptance_recorded` | `revoked` | `uncovered` |
| `version_not_accepted` | `revoked` | `uncovered` |
| `superseded_by_later_acceptance` | `superseded` | `uncovered` |
| `superseded_by_activation` | `superseded` | `uncovered` |
| `operation_in_flight` | `indeterminate` | `uncovered` |
| `replay_unresolved` | `indeterminate` | `uncovered` |

Each row is one decision path §5.3, §6.5, and §6.6 already distinguish, and the
mapping is total in both directions of use: every stored row projects onto the
generic code the enforcer saw in coverage, and no detail code exists without a
status. `operation_in_flight` is the only code `resolve_read_status`
substitutes during an overlay; `replay_unresolved` is the only one
materialization stores for an `unresolved` record. Coverage's generic
vocabulary is exactly the third column, so the enforcer's fixed codes are the
projection of this table and neither side can drift. §8.1 permits GitHub
numeric IDs and login snapshots here and forbids legal names, email addresses,
signer fields, and raw evidence, so the schema admits none of those.

A reader-authority shard maps `source_id` — the `event_id` of the canonical
source-creating event, which §6.6 also fixes as the sharding input — to a source
record carrying `kind` (`individual`, `snapshot`, or `continuous_team`), `team`
with its `organization_id` and `team_id` for the two team-derived kinds, and a
`subjects` map keyed by `github_user_id`.
...
and removing the last entry removes the source record, matching the source-union
rule that no boolean overwrite can hide another source. Because the source ID
stays the event ID, one source's accounts always live in one shard, which is
what makes a reader mutation touch one reader shard. The record's identifying
members are exactly those §6.6's private-read proof binds as
`authorizing_source`, so a proof can only name a source this shard contains, and
issuing one additionally requires the authenticated user to appear in that
source's `subjects`.
...
status, reason code, exemption-source kind, and reader-source kind. A snapshot
case authorizes several subjects in one event and proves each is independently
...
`subjects`. A privacy case asserts no status-detail row carries a legal name,
email address, signer field, or raw evidence member, and a mapping case asserts
every reason code projects onto the generic coverage code for the same subject.
...
Source-aware transitions additionally take the affected subjects' remaining
source union as an explicit authenticated input, for the reason M1-10 states:
coverage stores only the derived boolean, so an exemption or reader withdrawal
cannot be resolved from coverage alone. The Worker supplies it from the
records-side provenance under the `generation` binding mode and never infers it
from the coverage before-state.
...
withdraws the last and proves the entry is removed; a missing or stale source
union input is rejected rather than guessed. Negative cases cover wrong
