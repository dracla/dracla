...
rewrite. Version currency is therefore not stored per subject at all — see the
derived-state `active_agreement` member and the read-time derivation below.
...
- `agreements`, a map from `agreement_id` to that agreement's status entry,
  ...
  object `{"event_id", "kind"}` whose `kind` is `acceptance`, `exemption`,
  `revocation`, `none`, or `unresolved`. Every member is always present. The
  entry stores no `status` or `reason_code`; both are derived at read, for the
  reason given below.
...
event and kind (`acceptance`, `exemption`, `revocation`, `none`, or
`unresolved`) that the row rests on. Those values never change when another
subject's agreement is activated.

`resolve_read_status` produces the pair a reader is served, from the stored
facts, the `active_agreement` member above, and the operational state. Version
...
exactly `version`, `enforcement_scope`, `accepted_at`, `revoked_at`, and the
same `basis` object — §6.6's row fields minus the two that are the container
keys, with `status` derived at read rather than stored. `reason_code` and
`exemption_sources` are deliberately absent: §8.4 keeps the index to what the
mandated filters need, and the exact reason is status-detail data.
...
- `active_agreement`, the records-side mirror of the coverage active-agreement
  file: `agreement_id`, `active_version`, and `accepted_versions`. Activation
  updates this one small member and no shard, which is what keeps §6.5's O(1)
  transition O(1) on the derived side too. It advances no class generation,
  because no class's stored bytes changed;
...
generation, exact/private data separation, canonical/derived
...
`standing_rules` byte case fixes the map encoding with the full §5.1 team
object, and a digest case rejects a shard digest encoded as bare hex or
base64url rather than the `sha256:`-prefixed form. A dedicated case drives a
coverage
