...
exemption advancing `status_detail` alone while a flipping one advances both, a correction chain proving A2 replaces A1 in the single row without
...
`standing_rules` map exceeding `max_continuous_exemption_rules`, a reader
`materialization_cursor` exceeding `max_reader_cursor_entries` after repeated
membership churn, and a state file exceeding `max_state_ciphertext_bytes`. An
exemption-provenance case gives one subject several active sources and proves
both runtimes select the same latest source by `sequence`, that a withdrawal
leaving gaps keeps the selection well defined, and that a full materialization
and an incremental addition assign identical sequences. An add/withdraw/add
lifecycle empties the map entirely and proves the next addition takes its
sequence from `exemption_sequence_next` rather than restarting at zero, and
that its automation nonce derives from the prior materialization recorded in
