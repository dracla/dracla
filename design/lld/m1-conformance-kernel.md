...
- total subrequests per request, which §8.4 requires to stay below 50 and §5.3
...
limits are the §8.4 list's remaining two members; §9.2's capacity model is
their evidence, and a subrequest limit at or above 50 is rejected rather than
recorded. If a value changes a locked product behavior rather than bounding the
documented behavior, stop for design review.
...
remain Python-owned or belong to later milestone drivers.

**Open design task — the cross-runtime execution boundary is not yet fully
enumerated.** The bullet list above is M1's current TypeScript scope, not a
complete account of what a Worker must execute. Review has already identified
four edge-executed contracts it does not cover:

- the §5.4 canonical write path. The portal Worker is the canonical writer and
  must fully validate an event and its side-artifact package before the
  prepared cell, so it needs edge-executable event schema, identity,
  semantic-relation, and side-artifact validation. Authored by M1-5 and M1-9.
- incremental projection updates. §5.4 step 7 and §6.6 have the Worker
  read-modify-write the affected coverage and records-derived shards and state
  synchronously, and §6.6 states the normal Worker path never performs a full
  replay. M1-10 and M1-11 author only full materializers and deltas, so the
  incremental update-and-revalidate contract the edge actually executes is not
  yet specified or vectorized.
- §6.10.2 wrapped-key creation. Bootstrap sends each raw key to the portal and
  enforcer services, which return their capability-specific wrapped copies, so
  those Workers execute `wrap_key_copy`. The list above carries only keyring
  validation and unwrap.
- the §6.10.4 records-derived read path, whose derived-state and
  reader-authority validation M1-11 authors.

Each needs three decisions: which milestone authors the contract and its
vectors, which milestone implements the TypeScript side, and whether M1 closure
depends on it. Those are roadmap decisions, not implementer's choices, and
assigning them one at a time as review surfaces them is how the boundary got
under-specified. They are resolved together in one pass over §§5.3, 5.4, 6.6,
6.10.2, 6.10.4, and 7 before this LLD is locked.

Whatever the assignment, the corpus rule holds: a later milestone adds its
TypeScript implementation against this manifest and the authoring milestone's
vectors. It does not start a second corpus, re-specify a contract, or let one
runtime become the other's oracle.
