...
`enforcement_scope_requested`, `enforcement_scope_abandoned`,
`exemption_rule_configured`, `override`, `override_withdrawn`, and
`retry_requested` change no derived class: overrides live only in coverage, a
rule configuration changes nothing until it materializes, and the rest carry no
subject row. Reader events touch only `reader_authority`, which is what keeps
