...
rather than left to the table. A repeated reader withdrawal appended on the
stale path declares `reader_authority` affected even though the fold changes
nothing, because the edge provably cannot tell: the class that would reveal the
prior withdrawal is the stale one, and the prior event's path is not derivable.
This only ever arises on that path — against a current class the portal sees
the prior withdrawal and returns a terminal no-op without appending — so no
ambiguity exists anywhere else.
...
oversized, that withdrawing an already-withdrawn source returns a semantic no-op
when the portal can see the prior withdrawal, and that a second withdrawal
appended against a stale class folds to the same union under replay rather than
failing it. A departure case proves a `records_reader_materialized` withdraw is
deferred rather than appended while the class is stale, and that the live
membership recheck denies plaintext in the meantime.
