...
| `enforcement_scope_activated` | `derived_index`, `status_detail` |
...
4. `version_not_accepted` — an acceptance is current but its version is outside
   `accepted_versions`;
5. `superseded_by_activation`, then `superseded_by_later_acceptance`;
...
malformed operational state, a marker entry that does not match its fence, an unknown status value, stale
