...
A status-detail shard maps `github_user_id` to a subject record with two
members, because the two kinds of evidence have different scopes:

- `exemption_sources`, the subject-scoped provenance §4 assigns to the records
  key, each entry naming its `source_event_id`, `source_kind`, `basis`, and
  `instrument_ref`; and
...
empty and whose later source withdrawal still evaluates. A snapshot
