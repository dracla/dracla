...
A repeated *reader* withdrawal is deliberately not in that set. M1-5's
shrink-only rule makes it a §5.4 semantic no-op returned through
`terminal_noop_event_id`, so rejecting it here would both lose the replay-stable
no-op and put this slice in direct conflict with the Python action-form
contract on a shared case.
...
same semantic no-op in both runtimes rather than a rejection.
