...
update is skipped rather than attempted, so the append succeeds while that class
stays behind until repair; M1-18 states that path. A withdrawal of an already-withdrawn source is
an ordinary §5.4 semantic no-op, returned through `terminal_noop_event_id`
