...
proves the coverage delta advances `source.enc.json` while changing no decision,
exemption, or subject shard, and that a portal read immediately afterwards
satisfies the `canonical_sha` binding rather than failing closed. A crash fixture completes
