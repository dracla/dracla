...
source-creating event, which is also the sharding input — to a record carrying
`kind` (`individual`, `snapshot`, or `continuous_team`), `subject` for the two
account-scoped kinds, `team` with its `organization_id` and `team_id` for the
two team-derived kinds, the `source_event_id`, and `active: true`. Withdrawal
removes the entry rather than setting a false flag, matching the source-union
rule that no boolean overwrite can hide another source. The members are exactly
those §6.6's private-read proof binds as `authorizing_source`, so a proof can
name a source this shard actually contains.
...
status, reason code, exemption-source kind, and reader-source kind, and prove a
withdrawn reader source is absent rather than present with a false flag. A
