...
| `acceptance`, `revocation`, `agreement_activated` | `derived_index`, `status_detail` |
...
`agreement_published`, `config_updated`, `keyring_activated`,
`project_repository_owner_changed`, `project_succeeded`,
...
- `agreements`, a map from `agreement_id` to that agreement's status entry,
  ...
  the deciding `event_id`, `version`, and `accepted_at` and `revoked_at`. Every
  member is always present; the values each may take are fixed by `reason_code`
  in the table below rather than left free.
...
5. `superseded_by_later_acceptance`;
6. `version_not_accepted` — an acceptance is current and its version is outside
   `accepted_versions` for some reason other than a supersession;
7. `revoked_by_contributor`; and
8. `no_acceptance_recorded`.
...
Each row's provenance and timestamps are then determined by its `reason_code`,
so a replay and an incremental update cannot disagree while both satisfying the
field types:
...
| `exempt_source_active` | latest active exemption source event | `null` | `null` | `null` |
...
| `superseded_by_later_acceptance` | the superseding acceptance | superseded version | `null` | `null` |
| `superseded_by_activation` | the activation | superseded version | `null` | `null` |
...
`github_user_id` to a subject record with `login_snapshot`, `login_as_of`, and
an `agreements` map keyed by `agreement_id`. Each agreement entry carries
exactly `version`, `enforcement_scope`, `status`, `accepted_at`, and
`revoked_at` — §6.6's row fields minus the two that are the container keys.
...
Provenance and timestamps follow the same `reason_code`-driven table as
status detail, so an index row and its detail row can never disagree even though
the index does not store the code. `enforcement_scope` is the project's
effective scope at materialization, copied into each entry so a bounded page can
satisfy §6.6's mandated filters without another read; it is project-level truth
resolved by routing, never per-subject truth, and a materialization refreshes it
...
- `shard_digests`: one member per class, each a map keyed by the zero-padded
  ...
  whose value is the SHA-256 of that shard's encrypted envelope. Every class
  carries all 32 keys, and an unchanged shard's digest is carried forward;
...
§5.4 cell before preparation; `max_shard_ciphertext_bytes` is per records-derived
class and is checked against a selected envelope's ciphertext length, with the
class taken from the artifact's §4 identity; `max_reader_source_members` bounds
one reader source's `subjects` map; `max_bulk_shards_per_operation` bounds one
bulk fan-out; `max_mutation_retries` bounds §5.2's retry loop;
`max_subrequests_per_request` bounds one Worker invocation; and
`max_continuous_reader_rules` bounds active continuous reader rules.
`max_shard_ciphertext_bytes` has no coverage member, which is what makes a
coverage size limit unrepresentable rather than merely discouraged.
