...
One case cannot be decided by effect at write time, and it is stated rather than
left to the table. A repeated reader withdrawal appended on the stale path
declares `reader_authority` affected even though the fold changes nothing,
because the edge provably cannot tell: the class that would reveal the prior
withdrawal is the stale one, and the prior event's path is not derivable. This
only ever arises on that path — against a current class the portal sees the
prior withdrawal and returns a terminal no-op without appending — so no
