...
invocation; `max_continuous_reader_rules` bounds active continuous reader rules;
`max_continuous_exemption_rules` bounds active continuous exemption rules, whose
`standing_rules` entries would otherwise grow without limit inside a file every
derived read must fetch and validate; `max_reader_cursor_entries` bounds one
reader source's `materialization_cursor`, which accumulates across membership
churn rather than tracking concurrent membership; and
...
rejected before its canonical event is appended, exactly as §6.10.4 does for the
eleventh reader rule. `max_shard_ciphertext_bytes` has no coverage member, which is what makes
...
membership churn, and a state file exceeding `max_state_ciphertext_bytes`. An exemption-provenance case gives
...
automation nonce derives from the withdrawal recorded in
