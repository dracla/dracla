...
already-withdrawn source is an ordinary §5.4 semantic no-op, returned through
`terminal_noop_event_id` rather than treated as corruption.
...
oversized, and that withdrawing an already-withdrawn source returns a semantic
no-op rather than a corruption error.
