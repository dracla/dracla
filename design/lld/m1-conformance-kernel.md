...
therefore not in the row. A read does not name the superseding activation at all. The
active-agreement object carries only `activation_event_id` for the most recent
activation, which after a superseding V2 followed by a non-superseding V3 is V3
— not the activation that invalidated V1 — so reporting it would misattribute
the supersession. Identifying the exact activation requires canonical history,
and no bounded read is specified for it; `superseded_by_activation` states that
an activation invalidated this version, which is what the status and reason are
for. The
