...
when the subject's exemption flag is set and `no_acceptance_recorded` otherwise.
...
for the marker and fence, so it costs no extra resolution and cannot be stale
against it. `worker-portal` holds both keys, so no capability changes. Version
...
closed rather than reporting the former version, an exemption layered over a
...
`max_subrequests_per_request` of 50 or more. A `standing_rules` byte case fixes
