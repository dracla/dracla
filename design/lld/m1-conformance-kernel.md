...
that the row rests on; `event_id` is `null` exactly when `kind` is `none`. Those values never change when another
...
restarting at zero. A subject exempted before any agreement existed and then
un-exempted proves both runtimes serve `no_acceptance_recorded` with a `null`
`event_id` and a `none` basis rather than diverging on absent provenance. A scope case proves a scope change rewrites no index or
