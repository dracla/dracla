...
Every M1-12 derived triple round-trips byte for byte. Because stored bytes carry
no live trigger, the triples are identical under every operational state, and a
separate overlay suite drives the read path through each one. Cases cover all shard
