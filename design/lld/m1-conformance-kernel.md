...
status. `operation_in_flight` is the only code `resolve_read_status` substitutes
during an overlay; `replay_unresolved` is the only one materialization stores
for an `unresolved` record. Coverage's generic vocabulary is exactly the third
column, so the enforcer's fixed codes are the projection of this table and
neither side can drift. §8.1 permits GitHub numeric IDs and login snapshots here and
