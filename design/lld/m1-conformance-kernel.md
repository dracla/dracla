...
already-withdrawn source is an ordinary §5.4 semantic no-op wherever the portal
can see the prior withdrawal. Where it cannot — a stale `reader_authority` class
hides it, and the prior withdrawal's event path is not derivable from a random
operation nonce — the second withdrawal may append, and M1-7 folds it as an
idempotent no-op rather than corruption. Either way the resulting union is the
same and replay stays deterministic, and the append path is never blocked on
evidence the Worker cannot bound.
...
A repeated *reader* withdrawal is deliberately not in that set, and it has two
outcomes rather than one. Where the portal can see the prior withdrawal — the
`reader_authority` class is current — it is a §5.4 semantic no-op returned
through `terminal_noop_event_id`. Where it cannot, because that class is stale
and the prior withdrawal's event path is not derivable from a random operation
nonce, the edge does not manufacture a terminal identity it has no way to
obtain: the submission appends, and M1-7 folds it as an idempotent no-op.

Rejecting either outcome here would lose the replay-stable no-op in the first
case and, in the second, demand evidence the Worker cannot bound — putting this
slice in direct conflict with M1-5 on a shared case.
...
succeeded-project action. A repeated reader withdrawal is asserted to return the
same semantic no-op in both runtimes against a current `reader_authority` class,
and against a stale one to append in both runtimes and fold to the same union
under replay — never a rejection in either case.
