...
- `login_snapshot` and `login_as_of`, the recorded GitHub login and the instant
  ...
  Both sources are needed. An administrator who exempts or authorizes a subject
  records that subject's `login_snapshot` in the event, and the named-subject
  view must render it, so an actor-only rule would leave the pair `null`
  indefinitely for a subject who has never acted. An actor login is never
  attributed to a different subject: an administrator's own login updates only
  the administrator's record. Both members stay `null` until some event carries
  the subject's login. Without this rule a full replay and an incremental update
  could each pick a different historical login and still satisfy the field
  schema, producing divergent shard bytes;
...
Each row's provenance and timestamps are determined by its `reason_code`, so a
replay and an incremental update cannot disagree while both satisfying the field
types:
...
reader-authority bytes for every
...
empty and whose later source withdrawal still evaluates. Provenance cases fix
the `event_id`, `version`, and timestamp values for every `reason_code` row and
prove a revoked entry clears `accepted_at` while `version_not_accepted` keeps
it, that an index entry and its detail entry never disagree, and that
`derived/state.enc.json` carries all 32 digest keys per class with unchanged
shards carried forward. A named-subject case
...
members stay `null` only until some event carries that subject's login. Exemption-source byte cases fix
