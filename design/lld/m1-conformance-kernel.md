...
the Worker enumerates them from those shards. Reader events touch only `reader_authority`, which is what keeps
§6.6's one-shard-plus-state budget true.
...
- `standing_rules`, the active continuous exemption rules, each carrying its
  `rule_event_id`, `team`, `basis`, and `instrument_ref`. A withdrawn rule is
  removed rather than flagged, matching the source-union rule. This member is
  guarded by the `status_detail` generation, since a rule's effects are
  status-detail data;
...
32 digest keys per class with unchanged shards carried forward. A named-subject
