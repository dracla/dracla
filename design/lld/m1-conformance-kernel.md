...
- `validate_side_artifact_package(event, artifacts) -> None`.
...
- **event-determined bytes.** Where the event fixes the artifact exactly — the
  project configuration, which is the JCS of `payload.project_configuration`;
  the agreement metadata, for which §5.1 makes the event canonical; and the
  materialization generations, which must be the exact successor of the prior
  value for the classes this event changes — the validator derives the expected
  bytes and requires byte equality. A caller cannot substitute different
  content that merely hashes consistently.
...
An artifact whose bytes disagree with the event fails closed, as does a missing,
extra, misordered, or duplicate entry. This is the contract M1-15 ports, so the
Worker proves the package before preparing rather than trusting its own
assembly.
...
self-consistent, a wrong successor generation, a snapshot whose digest or path
disagrees with the event, and missing, extra, or duplicated entries.
...
- `validate_derived_projection(projection, *, generations, encrypted_shards,
  profile, operation_state) -> None`; and
...
**Derived reads validate live operational state, not only generations.** §6.6
defines `indeterminate` in the present tense — the subject *sits in*
`inflight.ops` — so it is a live condition, and the coverage marker and fence
that decide it live in a different repository from the `derived` branch that
stores the row. Canonical-versus-derived generation agreement does not close
that gap: between marker open and canonical append the generations still match,
so digest and generation checks alone would serve the previous `current` or
`revoked` row, and after the marker closes they would keep serving a stale
`indeterminate` one.

`resolve_read_status` therefore decides the status a reader is served, and
`validate_derived_projection` requires the operational state a read resolved:

- a subject covered by an open marker entry or a covering `mutation` fence is
  served `indeterminate` regardless of the stored row;
- a stored `indeterminate` row with no covering operational state means the
  class is stale rather than authoritative, so the read fails closed and
  schedules repair as §6.6 requires rather than inventing a status; and
- `success_reserved` with an empty marker covers nothing and leaves every stored
...
Missing, unreadable, or schema-invalid operational state denies the read.
...
before the derived write the class fails closed on generation mismatch; after
the marker closes but before re-materialization a stored `indeterminate` row
fails closed rather than being served; and under `success_reserved` with an
empty marker every stored status is served unchanged.
...
and `validate_side_artifact_package`.
...
and applies the live operational state to every row it serves, because §6.6
makes `indeterminate` a present-tense condition whose deciding state lives in
the other repository. A subject under an open marker or covering `mutation`
fence reads `indeterminate` whatever the shard stores; a stored `indeterminate`
row with no covering state fails the read closed and schedules repair. Matching
...
index, and incomplete fan-out. Cross-branch fixtures run both runtimes through
the crash windows above and require identical served statuses and identical
