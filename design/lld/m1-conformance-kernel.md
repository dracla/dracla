...
only the enum value. `ResolvedStatus` therefore carries both the served enum and
the served reason, and a read overlays both: when a marker or `mutation` fence covers a
