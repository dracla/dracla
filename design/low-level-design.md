...
| M1 — conformance kernel | [`design/lld/m1-conformance-kernel.md`](lld/m1-conformance-kernel.md) | Detailed draft | M1-1 and M1-2 landed; M1-3 through M1-19 planned |
...
- Python owns the authoritative models: the closed schemas, canonical byte
  contracts, identity derivation, semantic validation, the full replay fold,
  the reference materializers for every projection, export semantics, the
  reconciler, recovery/reporting logic, and the CLI. Every shared contract is
  specified, implemented, and given its expected bytes in Python first.
- Python also generates the shared vector corpus. A reference materializer is
  the oracle for the incremental edge path: the generator emits before-state,
  input, and after-state triples, and the edge implementation must reproduce
  the after-state exactly. Python therefore needs no second incremental
  implementation, and the two runtimes cannot drift into disagreeing folds.
- TypeScript owns every contract a Worker must itself execute: §5.4 event and
  side-artifact package validation on the canonical write path, §6.10.2
  wrapped-key creation and unwrap, §5.3/§5.4 incremental coverage updates,
  §6.6 incremental records-derived updates, and §6.10.4 portal read
  validation, together with the byte, envelope, action-form, and
  release-profile contracts those paths depend on.
- TypeScript never folds complete canonical history, never authors a contract,
  and never regenerates a vector. It re-implements the exact subset the edge
  executes and is tested only against Python-generated expected bytes.
- Not folding history is not the same as trusting the caller. Where a write's
  legality depends on prior history, the Python contract names the exact
  authenticated artifact or event path that decides each relation at the
  confirmed head, and the Worker reads and verifies it before writing. Replay
  independently rechecks every such relation, so the edge gate is never the
  sole authority.
- Routing state, signed KV projections, and the Durable Object gate are M4's,
  not M1's. They are Worker-side but are not shared byte contracts, so they
  are owned by the enforcement milestone rather than the conformance kernel.
- One checked-in corpus serves both runtimes. Independent runtime tests consume
  the same input and expected bytes; one runtime's output is never the other's
  test oracle, and no milestone starts a second corpus.
...
Review and lock the M1 LLD, then deliver M1-3 through M1-19 in order. M1-3 and
M1-4 are logically independent after the landed foundations but remain
sequential commits in one review stack. M1-3 through M1-11 author the contracts
in Python, M1-12 freezes the corpus, M1-13 through M1-18 implement the edge
subset in TypeScript, and M1-19 closes the milestone. M1 closes only after the
shared Python and TypeScript conformance gate passes.
