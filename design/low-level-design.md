...
- Python owns the canonical replay fold, export semantics, the reconciler,
  recovery/reporting logic, and the CLI. It is the only runtime that folds
  complete canonical history, and it is the authoring runtime for every
  canonical contract: each one is specified, implemented, and given its vectors
  in Python first.
- TypeScript owns thin Cloudflare request paths and implements only the
  contracts a Worker must itself execute. That is not limited to bytes, forms,
  routing, and coverage. §5.4 makes the portal Worker the canonical writer and
  requires an event and its complete side-artifact package to be fully
  validated before the prepared cell is written, so the write path needs
  edge-executable event schema, identity, semantic-relation, and side-artifact
  validation. §9 places Python in Actions and the CLI, not at the edge, so
  there is no synchronous Python boundary for a Worker to call and D8's thin
  edge cannot be read as "no validation here."
- Thin means the edge folds no canonical history and owns no contract: it
  re-implements the exact subset it must execute and nothing more. Replay,
  export, reconciliation, and recovery stay Python-only.
- Every contract a second runtime executes uses one checked-in vector corpus.
  Independent runtime tests consume the same input and expected bytes; one
  runtime's output is never the other's test oracle. A milestone that adds an
  edge implementation of a Python-authored contract adds it against that
  contract's existing vectors rather than starting a second corpus.
