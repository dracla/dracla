...
`checks_requested` event, DraCLA MUST resolve exactly one current pull request
associated with that queue entry using supported GitHub data, evaluate that
pull request's current opener and commit authors against the latest canonical
CLA records, and report on the merge-group commit. Missing or ambiguous
association MUST produce a non-passing unavailable result rather than a guess.

Every pull request MUST independently pass the ordinary DraCLA check before it
enters the queue and MUST receive its own merge-group evaluation when GitHub
requests checks for that queue entry. A temporary merge-group branch may also
contain changes from entries ahead of it; DraCLA MUST NOT reconstruct or
re-evaluate those preceding pull requests in the current entry's check. Their
own independently completed checks remain authoritative for those entries.
The merge-group result is the authoritative CLA decision for its associated
pull request's landing attempt.

That decision is made when the associated pull request's merge-group check
successfully completes. A later signing, revocation, agreement, exemption, or
configuration change MUST NOT retroactively rewrite the completed decision.
If GitHub rebuilds the queue entry or sends another `checks_requested` event
before landing, the new evaluation supersedes the earlier result and MUST use
the canonical state observed when the new check runs. References in this
document to a future merge decision use this decision time, not the later
instant when GitHub physically updates the target branch.
...
The authoritative merge-group check MUST freshly evaluate its associated pull
request's current opener and commit authors against the canonical CLA state
observed when that check runs; it MUST NOT merely copy the earlier ordinary
pull-request result. A later GitHub rebuild or new `checks_requested` event
causes another fresh evaluation for that entry.
...
- a required merge-group check that observes a revocation or agreement change
  made after the associated pull request's ordinary check;
- a two-entry queue in which each pull request receives its own merge-group
  evaluation and the later cumulative temporary branch does not require
  rediscovery of the preceding pull request;
- a queue entry whose merge-group check passes before a revocation or agreement
  change but physically lands afterward, preserving the completed decision;
  and the same sequence with a GitHub rebuild or new `checks_requested` event,
  requiring a fresh decision from the changed canonical state;
...
Round 17 of the HLD review found that Revision 12 required DraCLA to discover
every pull request represented in a cumulative merge-group commit even though
GitHub's documented webhook contract does not provide that complete list. The
project owner selected the established per-pull-request queue-entry model used
by comparable CLA systems:

- **Per-entry enforcement (`REQ-CHECK-3`).** Every pull request independently
  passes the ordinary DraCLA check before queue entry and receives a fresh
  authoritative evaluation when GitHub requests checks for its merge-group
  entry. DraCLA posts that result on the temporary merge-group commit.
- **No cumulative reconstruction (`REQ-CHECK-3`).** The current entry does not
  rediscover or re-evaluate preceding pull requests whose changes GitHub also
  includes in its cumulative temporary branch; those entries retain their own
  independently completed decisions.
- **Fresh current-PR state (`REQ-CHECK-4`).** The merge-group handler resolves
  exactly one associated pull request and freshly checks its opener and commit
  authors against canonical CLA state. Missing or ambiguous association fails
  non-passing, and a rebuilt entry is evaluated again.
- **Bounded decision time (`REQ-CHECK-3`).** A pull request's authoritative CLA
  decision occurs when its own merge-group check completes. Later canonical
  changes do not retroactively rewrite that result, but a rebuilt entry or new
  check request observes the new state. This accepts the small interval between
  the completed check and GitHub's physical merge.

*Rationale.* CLA enforcement is a property of each pull request, while GitHub's
merge queue is responsible for composing already-gated entries. This removes
an undocumented all-membership dependency without weakening the fresh check on
the pull request currently asking to land.
