...
Status: Locked
Date: 21 August 2026
Revision: 6 — resolves the two findings reopened by round 4 of the 21 August
2026 deep-design-review loop; see section 20.
...
Every mutating signing or administrative submission MUST carry a canonical
idempotency key. The key MUST remain stable across retries of one submission
and MUST be fresh for a new explicit action, even when its submitted values are
identical to an earlier action. Reuse of one key with different authenticated
actor, operation type, target, or payload MUST fail as a conflict and MUST NOT
append an event. The event record MUST preserve the key and define an
unambiguous mapping between that key and the event identity or path.
...
An authenticated contributor MUST be able to revoke a current acceptance for a
specific agreement from the same project-facing portal used to inspect or sign
that agreement. The initial release MUST NOT provide an implicit project-wide
...
Revocation MUST append a timestamped event that identifies the agreement and
acceptance being revoked. It MUST NOT delete or rewrite the acceptance or
affect acceptances for other agreements.

### REQ-REV-4: Enforcement after revocation

After revocation, DraCLA MUST report the contributor as not currently covered
by that agreement for new merge decisions until the contributor signs an
acceptable version of that agreement again. Existing accepted contributions,
other agreements, and historical evidence remain unchanged.
...
- refusal to bind a repository or organization-wide selector that would place
  ...
  and documentation that independent domains cannot enforce global uniqueness;
